...
BEGIN
	...
	DECLARE @pmo_number VARCHAR(MAX) = '58872'

	DECLARE @src_org_code VARCHAR(MAX) = 'ORCH'
	DECLARE @dst_org_code VARCHAR(MAX) = 'HAND'

	DECLARE @src_prod VARCHAR(MAX) = '[pccsql-use2-prod-w27-cli0023.851c37dfbe45.database.windows.net].[us_orch_multi]'
	DECLARE @dst_prod VARCHAR(MAX) = '[pccsql-use2-prod-w24-cli0023.ce22455c967a.database.windows.net].[us_hand_multi]'

	DECLARE @src_test_org_code VARCHAR(MAX) = 'usei630'
	DECLARE @dst_test_org_code VARCHAR(MAX) = 'usei587'

	DECLARE @golive_date VARCHAR(MAX) = 'December 13, 2021'
	DECLARE @golive_day VARCHAR(MAX) = 'Monday'
	DECLARE @golive_starttime VARCHAR(MAX) = 'Monday, December 13th, 5PM'
	DECLare @golive_endtime varchar(max) = '2AM'
	DECLARE @golive_time VARCHAR(MAX) = '5PM to 2AM'
	DECLARE @offline_minutes VARCHAR(MAX) = '5'
	DECLARE @facility_names VARCHAR(MAX) = 'The Orchards of Saxonburg '

	DECLARE @all_SRC_facID VARCHAR(MAX) = '6,'
	DECLARE @all_DST_facID VARCHAR(MAX) = '68,'
	DECLARE @case_number VARCHAR(MAX) = '[NOT CREATED YET]' -- netsuite photos & online docs case

	DECLARE @tst VARCHAR(MAX) = '1085'
	...
	DECLARE @deadline varchar(200) = 'Monday, December 13th'
