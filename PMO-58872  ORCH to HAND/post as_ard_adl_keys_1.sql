...
SELECT src_id, dst_id FROM us_hand_multi.dbo.EICase588721as_std_assessment

CREATE TABLE #map_as_ard_planner (src_id INT PRIMARY KEY, dst_id INT NOT NULL)
INSERT INTO #map_as_ard_planner
SELECT src_id, dst_id FROM us_hand_multi.dbo.EICase588721as_ard_planner

DROP TABLE IF EXISTS #temp
SELECT DISTINCT 
	  ISNULL(c.dst_id, ard_planner_id)  ard_planner_id
    , ISNULL(b.dst_id, std_assess_id) std_assess_id
    , [question_key]
    , [resp_value]
    , [source_id]
	INTO #temp
FROM [pccsql-use2-prod-ssv-tscon06.b4ea653240a9.database.windows.net].[test_usei630].dbo.as_ard_adl_keys a
...
INSERT INTO us_hand_multi.dbo.as_ard_adl_keys
(
    ard_planner_id
    , std_assess_id
    , question_key
