...
BEGIN
	...
	DECLARE @pmo_number VARCHAR(MAX) = '58910'


	DECLARE @src_org_code VARCHAR(MAX) = 'GBSL'
	DECLARE @dst_org_code VARCHAR(MAX) = 'GDN'
	...
	DECLARE @src_test_org_code VARCHAR(MAX) = 'usei548'
	DECLARE @dst_test_org_code VARCHAR(MAX) = 'usei1072'

	DECLARE @golive_date VARCHAR(MAX) = 'December 20, 2021'
	DECLARE @golive_day VARCHAR(MAX) = 'Monday'
	DECLARE @golive_starttime VARCHAR(MAX) = 'Monday, December 20th, 7PM'
	DECLare @golive_endtime varchar(max) = '7PM'
	DECLARE @golive_time VARCHAR(MAX) = '7PM to 4AM'
	DECLARE @offline_minutes VARCHAR(MAX) = '5'
	DECLARE @facility_names VARCHAR(MAX) = 'Belmont Nursing and Rehab Center'

	DECLARE @all_SRC_facID VARCHAR(MAX) = '1067'
	DECLARE @all_DST_facID VARCHAR(MAX) = '59'
	DECLARE @case_number VARCHAR(MAX) = '' -- netsuite photos & online docs case

	DECLARE @tst VARCHAR(MAX) = '1132'
	...
	DECLARE @implementers varchar(200) = 'Tammi & Matt'
	DECLARE @deadline varchar(200) = 'Monday, December 20th'
