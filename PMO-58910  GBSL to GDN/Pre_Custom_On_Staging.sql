...
update sec_role 
set description = 'GBSL-' + description 
--select * 
from sec_role 
where (system_field <> 'Y' or system_field is null)
and description not like 'GBSL%'



update  pcc_staging_db58910.dbo.mergeTablesMaster
set  queryfilter= CONCAT(queryfilter,' ', ' AND event_id=1 AND action_id NOT IN(3,4)')
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58910_AdminPickList$] a
LEFT JOIN test_usei548.dbo.common_code AS src on src.item_id = a.src_item_id
LEFT JOIN [pccsql-use2-prod-w25-cli0021.2d62ac5c7643.database.windows.net].[us_gdn_multi].dbo.common_code AS dst on dst.item_id = a.Map_DstItemId
WHERE 
	(
		...
		AND a.Map_DstItemId NOT IN --remove any dupilicate many to one mappings (Any If_merged not As_is or N)		--
			(
				...
				(
					...
					FROM [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58910_AdminPickList$]
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58910_ResidentIdentifier$] a
JOIN [pccsql-use2-prod-ssv-tscon05.b4ea653240a9.database.windows.net].[test_usei548].dbo.id_type AS src on src.id_type_id = a.srcIdTypeId
JOIN [pccsql-use2-prod-w25-cli0021.2d62ac5c7643.database.windows.net].[us_gdn_multi].dbo.id_type AS dst on dst.id_type_id = a.map_dst_typeid
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58910_UserDefinedData$] a
JOIN [pccsql-use2-prod-ssv-tscon05.b4ea653240a9.database.windows.net].[test_usei548].dbo.user_field_types AS src on src.field_type_id = a.srcFieldTypeId
JOIN [pccsql-use2-prod-w25-cli0021.2d62ac5c7643.database.windows.net].[us_gdn_multi].dbo.user_field_types AS dst on dst.field_type_id = a.map_dst_typeid
WHERE  a.srcFieldName IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dst_typeid IS NOT NULL --total count of mapping provided by implementer  
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58910_Clinical_common_code$] a
JOIN test_usei548.dbo.common_code AS src on src.item_id = a.src_item_id
JOIN [pccsql-use2-prod-w25-cli0021.2d62ac5c7643.database.windows.net].[us_gdn_multi].dbo.common_code AS dst on dst.item_id = a.Map_DstItemId
WHERE 
	(
		a.pick_list_name IS NOT NULL -- total mapping provided by analyst to implementer							--
		AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer							--
		...
		AND a.Map_DstItemId NOT IN --remove any dupilicate many to one mappings (Any If_merged not As_is or N)		--
			(
				...
				(
					select map_dstitemid 
					FROM [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58910_Clinical_common_code$]
					WHERE pick_list_name IS NOT NULL -- total mapping provided by analyst to implementer	
						AND map_dstitemid IS NOT NULL --total count of mapping provided by implementer		
						AND src_item_id IS NOT NULL --eliminate any incorrect provided mappings	
						AND ISNUMERIC(map_dstitemid) = 1
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58910_Clinical_Advanced$] a
inner join [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58910_Clinical_Advanced$] b 
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58910_Clinical_Advanced$] a
JOIN [pccsql-use2-prod-ssv-tscon05.b4ea653240a9.database.windows.net].[test_usei548].dbo.pho_administration_record AS src on src.administration_record_id = a.src_id
JOIN [pccsql-use2-prod-w25-cli0021.2d62ac5c7643.database.windows.net].[us_gdn_multi].dbo.pho_administration_record AS dst on dst.administration_record_id = a.Map_DstItemId
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58910_Clinical_Advanced$] a
JOIN [pccsql-use2-prod-ssv-tscon05.b4ea653240a9.database.windows.net].[test_usei548].dbo.pho_order_type AS src on src.order_type_id = a.src_id
JOIN [pccsql-use2-prod-w25-cli0021.2d62ac5c7643.database.windows.net].[us_gdn_multi].dbo.pho_order_type AS dst on dst.order_type_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
--SELECT distinct a.src_id, a.Map_DstItemId, src.description , dst.description
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58910_Clinical_Advanced$] a
JOIN [pccsql-use2-prod-ssv-tscon05.b4ea653240a9.database.windows.net].[test_usei548].dbo.pn_type AS src on src.pn_type_id = a.src_id
JOIN [pccsql-use2-prod-w25-cli0021.2d62ac5c7643.database.windows.net].[us_gdn_multi].dbo.pn_type AS dst on dst.pn_type_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
--SELECT distinct a.src_id, a.Map_DstItemId, src.description , dst.description, src.short_description, dst.short_description
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58910_Clinical_Advanced$] a
JOIN [pccsql-use2-prod-ssv-tscon05.b4ea653240a9.database.windows.net].[test_usei548].dbo.cr_std_immunization AS src on src.std_immunization_id = a.src_id
JOIN [pccsql-use2-prod-w25-cli0021.2d62ac5c7643.database.windows.net].[us_gdn_multi].dbo.cr_std_immunization AS dst on dst.std_immunization_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
--SELECT distinct a.src_id, a.Map_DstItemId, src.description , dst.description
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58910_Clinical_Advanced$] a
JOIN [pccsql-use2-prod-ssv-tscon05.b4ea653240a9.database.windows.net].[test_usei548].dbo.cp_std_shift AS src on src.std_shift_id = a.src_id
JOIN [pccsql-use2-prod-w25-cli0021.2d62ac5c7643.database.windows.net].[us_gdn_multi].dbo.cp_std_shift AS dst on dst.std_shift_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
--select * from [pccsql-use2-prod-w25-cli0021.2d62ac5c7643.database.windows.net].[us_gdn_multi].dbo.mergeTablesmaster where tablename = 'inc_std_pick_list_item'
...
--SELECT distinct a.src_id, a.Map_DstItemId, src.description , dst.description
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58910_Clinical_Advanced$] a
JOIN [pccsql-use2-prod-ssv-tscon05.b4ea653240a9.database.windows.net].[test_usei548].dbo.inc_std_pick_list_item AS src on src.pick_list_item_id = a.src_id
JOIN [pccsql-use2-prod-w25-cli0021.2d62ac5c7643.database.windows.net].[us_gdn_multi].dbo.inc_std_pick_list_item AS dst on dst.pick_list_item_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
-----------------------------common_code_audit__itemId_effectiveDate_PK_CL_IX------
