USE [pcc_staging_db008679_1]
GO

/****** Object:  StoredProcedure [operational].[sproc_facacq_mergeCreateTables]    Script Date: 1/20/2022 7:15:48 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			IF @tablename ='as_assessment'
			BEGIN
			SELECT @SQL ='IF OBJECT_ID('''+@dbStag + @prefix +'as_assessment'') IS NOT NULL AND NOT EXISTS (SELECT * FROM sys.indexes
						WHERE name=''idx_' + @prefix +'as_assessment_src_id_corporate'')
						BEGIN
						CREATE  NONCLUSTERED INDEX  idx_'+  @prefix +'as_assessment_src_id_corporate ON '+@dbStag + @prefix +'as_assessment ([src_id],[corporate])
						END 
						'
			EXEC(@sql)
			END
			
			IF @tablename ='pho_schedule_details'
			BEGIN
			SELECT @SQL ='IF OBJECT_ID('''+@dbStag + @prefix +'pho_schedule_details'') IS NOT NULL AND NOT EXISTS (SELECT * FROM sys.indexes
						WHERE name=''idx_' + @prefix +'pho_schedule_details_src_id'')
						BEGIN
						CREATE  CLUSTERED INDEX  idx_'+  @prefix +'pho_schedule_details_src_id ON '+@dbStag + @prefix +'pho_schedule_details ([src_id])
						END 
						'
			EXEC(@sql)
			END
