...
BEGIN
	...
	SET @vPMOGroupID = 1785

	/*------Job Fail Email Recipient------*/
	SET @vToEmailAddress = N'alexander.morcet@pointclickcare.com;Tech Services Customer Maintenance <TechServicesCustomerMaintenance@pointclickcare.com>'

	/*------External Email Notification Recipient, for start of the run, email address only, semicolon separated------*/
	SET @EmailrecipientsExt = 'Shannon MacKinnon <shannon.m@pointclickcare.com>; Ari  Grinspan <agrinspan@wecarehcc.com>; Patrick Fraccola <pfraccola@wecarehcc.com>;TS Fac Acq Configuration <TSFacAcqConfig@pointclickcare.com>; Ashlee Moss <Ashlee.Moss@pointclickcare.com>; Raju Sivapalan <Raju.Sivapalan@pointclickcare.com>; Lindsey English <Lindsey.English@pointclickcare.com>; Cody McCormick <Cody.McCormick@pointclickcare.com>'
	...
	SET @action_codes_mapping_table			= '[vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E007340_ActionCodes$]'
	SET @status_codes_mapping_table			= '[vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E007340_StatusCodes$]'
	SET @upload_categories_mapping_table	= '[vmuspassvtsjob1.pccprod.local].FacAcqMapping.[dbo].[E007340_UploadCategories$]'
	SET @room_rate_type_mapping_table		= '[vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E007340_RoomRateType$]'
	...
	/*------If 'Y', payer mapping is inculuded and with naming convention, on FacAcqMapping: 
			PMO007340_EICase0073404_payermapping_fac_4_to_6-----*/
	SET @if_include_payer_mapping			= 'Y'

	SET @prefix_for_UDA						= 'WSGC-'
	SET @prefix_for_Care_Plan_Library		= 'WSGC-'
	SET @CP_Library_to_exclude_from_prefix	= '8,9'
	SET @suffix_for_order_libraries			= '_'
	SET @suffix_for_sec_user_loginname		= 'WSGC'
	...
	IF @vProdRun = 'Y'
	BEGIN
		SET @vPagerDutyOperator = 'FacAcqPagerTS10'
		SET @vPagerDutyEmail = lower(right(@vPagerDutyOperator, 4)) + '@pccs.pagerduty.com'
	END
	ELSE
	BEGIN
		SET @vPagerDutyOperator = 'FacAcqPagerTS10'
	...
	BEGIN TRY
		...
		BEGIN
			...
			BEGIN --Start CurCase
				...
				SET @SqlCommand = @SqlCommand + 'INNER JOIN EICase' + @vCaseNo + 'as_ard_planner c ON c.src_id = a.ard_planner_id'+CHAR(13)+CHAR(10)+CHAR(13)+CHAR(10) END
			...
			SET @Recipient = iif(@vProdRun = 'N', @vPagerDutyEmail, @vToEmailAddress)
...
----E7d: MMQ, E7e: MMA, E19a: IRM, E19b: IRM