...
BEGIN
	...
	DECLARE @pmo_number VARCHAR(MAX) = 'E007340'

	DECLARE @src_org_code VARCHAR(MAX) = 'WSGC'
	DECLARE @dst_org_code VARCHAR(MAX) = 'WEC'

	DECLARE @src_prod VARCHAR(MAX) = '[pccsql-use2-prod-ssv-tscon05.b4ea653240a9.database.windows.net].[test_usei549]'
	DECLARE @dst_prod VARCHAR(MAX) = '[pccsql-use2-prod-ssv-tscon10.b4ea653240a9.database.windows.net].[test_usei1063]'

	DECLARE @src_test_org_code VARCHAR(MAX) = 'usei549'
	DECLARE @dst_test_org_code VARCHAR(MAX) = 'usei1063'

	DECLARE @golive_date VARCHAR(MAX) = 'February 7, 2022'
	DECLARE @golive_day VARCHAR(MAX) = 'Monday'
	DECLARE @golive_starttime VARCHAR(MAX) = 'Monday, February 7, 9PM'
	DECLare @golive_endtime varchar(max) = '4AM'
	DECLARE @golive_time VARCHAR(MAX) = '9PM to 4AM'
	DECLARE @offline_minutes VARCHAR(MAX) = '5'
	DECLARE @facility_names VARCHAR(MAX) = 'Wesley Gardens Corporation '

	DECLARE @all_SRC_facID VARCHAR(MAX) = '4'
	DECLARE @all_DST_facID VARCHAR(MAX) = '6'
	DECLARE @case_number VARCHAR(MAX) = '' -- netsuite photos & online docs case

	DECLARE @tst VARCHAR(MAX) = '1101'
	...
	DECLARE @implementers varchar(200) = 'Ashlee & Raju'
	DECLARE @deadline varchar(200) = 'Wednesday, January 26'

	DECLARE @signature varchar(200) = 'Alexander Morcet'
	DECLARE @recepients AS NVARCHAR(max) = 'alexander.morcet@pointclickcare.com;gunald@pointclickcare.com'--except this, change this :D
