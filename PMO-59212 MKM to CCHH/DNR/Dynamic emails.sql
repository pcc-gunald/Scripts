...
BEGIN
	...
	DECLARE @pmo_number VARCHAR(MAX) = 'PMO-59212'

	DECLARE @src_org_code VARCHAR(MAX) = 'MKM'
	DECLARE @dst_org_code VARCHAR(MAX) = 'CCHH'

	DECLARE @src_prod VARCHAR(MAX) = '[pccsql-use2-prod-w26-cli0016.d9c23db323d7.database.windows.net].[us_mkm_multi]'
	DECLARE @dst_prod VARCHAR(MAX) = '[pccsql-use2-prod-w22-cli0021.4c4638f8e26f.database.windows.net].[us_cchh_multi]'
	...
	DECLARE @golive_starttime VARCHAR(MAX) = 'Wednesday, March 2, 9PM'
	DECLare @golive_endtime varchar(max) = '9PM'
	DECLARE @golive_time VARCHAR(MAX) = '9PM to 4AM'
	DECLARE @offline_minutes VARCHAR(MAX) = '15'
	DECLARE @facility_names VARCHAR(MAX) = 'ZMeadow Wind Health Care Center'

	DECLARE @all_SRC_facID VARCHAR(MAX) = '3'
	DECLARE @all_DST_facID VARCHAR(MAX) = '29'
	DECLARE @case_number VARCHAR(MAX) = '' -- netsuite photos & online docs case

	DECLARE @tst VARCHAR(MAX) = '1189'
	...
	DECLARE @implementers varchar(200) = 'Tammi & Delores'
	DECLARE @deadline varchar(200) = 'March 2, 2022'
