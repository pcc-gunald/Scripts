...
BEGIN
	...
	DECLARE @pmo_number VARCHAR(MAX) = '009215'


	DECLARE @src_org_code VARCHAR(MAX) = 'SNHC'
	DECLARE @dst_org_code VARCHAR(MAX) = 'CMSHC'

	DECLARE @src_prod VARCHAR(MAX) = '[pccsql-use2-prod-w21-cli0001.f352397924df.database.windows.net].[us_snhc_multi]'
	DECLARE @dst_prod VARCHAR(MAX) = '[pccsql-use2-prod-w21-cli0002.f352397924df.database.windows.net].[us_cmshc_multi]'

	DECLARE @src_test_org_code VARCHAR(MAX) = 'usei1029'
	DECLARE @dst_test_org_code VARCHAR(MAX) = 'usei41'

	DECLARE @golive_date VARCHAR(MAX) = 'February 17, 2022'
	DECLARE @golive_day VARCHAR(MAX) = 'Thursday'
	DECLARE @golive_starttime VARCHAR(MAX) = 'Thursday, February 17th, 11PM'
	DECLare @golive_endtime varchar(max) = '11PM'
	DECLARE @golive_time VARCHAR(MAX) = '11PM to 4AM'
	DECLARE @offline_minutes VARCHAR(MAX) = '5'
	DECLARE @facility_names VARCHAR(MAX) = 'Whispering Oak Place,Garden View Place,Apple Creek Place,Arbor Garden Place,Harmony Place'

	DECLARE @all_SRC_facID VARCHAR(MAX) = '48,53,72,87,88'
	DECLARE @all_DST_facID VARCHAR(MAX) = '86,87,88,89,90'
	DECLARE @case_number VARCHAR(MAX) = '' -- netsuite photos & online docs case

	DECLARE @tst VARCHAR(MAX) = '1231'
	...
	DECLARE @implementers varchar(200) = 'Tammi & Melodee'
	DECLARE @deadline varchar(200) = 'Thursday, February 17th'
