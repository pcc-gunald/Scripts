...
BEGIN
	BEGIN TRY
		...
		SET @EmailrecipientsInt = 'palvinder.singh@pointclickcare.com;'
...
--		SET @List = 'RESTORE DATABASE ' + @vStageDBName + ' FROM URL = ''' + @vStagingDBBackupForProdPath + '/' + @List
		...
		BEGIN --Start CurCase
			...
			IF EXISTS(SELECT 1 FROM @LoadEIMaster WHERE (',' + ModList + ',') LIKE '%,E13,%' and RunId = @vCurrentRunId)
				BEGIN 
					SET @SqlCommand = @SqlCommand + 'DELETE FROM PCC_GLOBAL_PRIMARY_usei23 WHERE TABLE_NAME = ''PHO_ADMIN_ORDER_AUDIT'';'+CHAR(13)+CHAR(10)
					SET @SqlCommand = @SqlCommand + 'DELETE FROM PCC_GLOBAL_PRIMARY_usei23 WHERE TABLE_NAME = ''PHO_RELATED_ORDER_AUDIT'';'+CHAR(13)+CHAR(10)+CHAR(13)+CHAR(10)
...
Completion time: 2022-03-01T10:50:05.7566476-05:00
