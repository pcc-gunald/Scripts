...
JOIN [pccsql-use2-prod-w20-cli0017.3055e0bc69f6.database.windows.net].us_vvhr.dbo.sec_user dst ON src.loginname = dst.loginname + 'GULF'
...
SELECT distinct src.long_username AS [Username in GULF]
     , src.loginname AS [Existing Login in GULF]
	 , dst.loginname + 'GULF' AS [New Login in NSPR]
	 , dst.long_username AS [Existing Username in NSPR]
     , dst.loginname AS [Existing Login in NSPR]
FROM R_SRCPROD.dbo.sec_user AS src INNER JOIN R_DSTTESTWSERVER.dbo.sec_user AS dst ON src.loginname = dst.loginname
WHERE  src.userid IN (SELECT userid FROM pcc_temp_storage.dbo.EIcaseR_CASENUMBER1_NSPR_sec_user_samelogin_different_name)
--SELECT src.long_username AS [Username in GULF]
--     , src.loginname + 'GULF' AS [New Login in NSPR]
--     , dst.long_username AS [Existing Username in NSPR]
--     , dst.loginname AS [Existing Login in NSPR]
--FROM [pccsql-use2-prod-w22-cli0006.4c4638f8e26f.database.windows.net].us_srz_multi.dbo.sec_user AS src
--    INNER JOIN sec_user AS dst ON src.loginname = dst.loginname
--WHERE src.userid IN (SELECT userid FROM pcc_temp_storage.dbo.EIcase010798798_NSPR_sec_user_samelogin_different_name)
------(2 row(s) affected)
