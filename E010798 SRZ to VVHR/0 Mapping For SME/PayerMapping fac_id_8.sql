DECLARE @srcFacID VARCHAR(100)		=	'8' --'40'
		,@dstFacID VARCHAR(100)		=	'13'
		,@srcServer VARCHAR(500)	=	'[pccsql-use2-prod-w22-cli0006.4c4638f8e26f.database.windows.net].us_srz_multi'
		,@dstServer VARCHAR(500)	=	'[pccsql-use2-prod-w20-cli0017.3055e0bc69f6.database.windows.net].us_vvhr'
		,@SQL VARCHAR(max)			=	'SELECT * FROM ('

BEGIN

--Riverbend Heights Health & Rehabilitation - Lexington, MO- SNF
--Select regional_id, * from [pccsql-use2-prod-w22-cli0006.4c4638f8e26f.database.windows.net].us_srz_multi.dbo.facility  where fac_id in (8)
--Select regional_id,* from [pccsql-use2-prod-w20-cli0017.3055e0bc69f6.database.windows.net].us_vvhr.dbo.facility  where fac_id in (13)
