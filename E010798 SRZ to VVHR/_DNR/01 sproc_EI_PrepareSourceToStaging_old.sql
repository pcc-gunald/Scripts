...
BEGIN
	...
	DECLARE @if_as_ard_adl_usei23s_post_insert varchar(1)		--- added Jan 18 2022, by Amardeep
	...
	/*
Source_DB_Version
test_usei1188 - DB Version: 4.4.9.4_Z                                         
Staging_DB_Version
pcc_staging_db010798 - DB Version: 4.4.9.4_Z                                         
Destination_DB_Version
test_usei23 - DB Version: 4.4.9.4_Z                                                                         

RunID	CaseNo	srcfacid	srcfacdeleted	srcfacname	srcfacaddress	dstfacid	dstfacdeleted	dstfacname	dstfacaddress	ModIncluded	ModExcluded
853	01079812	45	N	010798-Consulate Health Care of Vero Beach	1310 37th Street	216	N	010798-Consulate Health Care of Vero Beach	1310 37th Street	E1: Resident Identifier, Contact, E2: Staff, E2a: Medical Professional, E3: External Facility, E4: User Defined Data, E5: Room and Bed, E6: Census, E7a: MDS 2, E7b: MDS 3, E8: Diagnosis, E12a: Custom Care Plan, E11: Immunization, E9: Progress Note, E10: Vital, E13: Phys Orders, E21: Results Lab, Radiology, E22: Master Insurance	E14: Security Roles, E15: Security Users, E7c: Custom UDAs, E7d: MMQ, E7e: MMA, E12b: Care Plan Copy Library, E16: Alert, E17: Risk Management, E18: Trust, E19a: IRM, E19b: IRM, E20: Upload Files, E23: Notes
	*/
	
	/*------PMO Group ID, make sure the PMO Number set up is unique------*/
	SET @vPMOGroupID = 1839

	/*------Job Fail Email Recipient------*/
	SET @vToEmailAddress = N'Palvinder.singh@pointclickcare.com;'

	/*------External Email Notification Recipient, for start of the run, email address only, semicolon separated------*/
	SET @EmailrecipientsExt = 'Palvinder.singh@pointclickcare.com;'
	/*------External Email Notification Subject, for start of the run, will use default if left empty------*/
	SET @vExtEmailSubject = 'Friendly Reminder Email:PMO010798 Location Acquisition CMC to CIT'

	/*------Mapping Backup Tables, will not be applied if left empty, please do not create table name with single quotes------*/
	SET @action_codes_mapping_table			='[vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO010798_ActionCodes$]'
	SET @status_codes_mapping_table			= '[vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO010798_StatusCodes$]'
	SET @upload_categories_mapping_table	= ''
	SET @room_rate_type_mapping_table		='[vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO010798_RoomRateType$]'
	...
	PMO010798_EICase01079812_payermapping_fac_45_to_216
			PMO[PMO_NUMBER]_EICase[CASE_NUMBER]_payermapping_fac_[SRC_FAC_ID]_to_[DST_FAC_ID]-----*/
	SET @if_include_payer_mapping			= 'Y'

	SET @prefix_for_UDA						= 'CMC-'
	SET @prefix_for_Care_Plan_Library		= 'CMC-'
	SET @CP_Library_to_exclude_from_prefix	= ''
	SET @suffix_for_order_libraries			= '_'
	SET @suffix_for_sec_user_loginname		= 'CMC'
	...
	SET @current_resident_as_of				= '2022-03-01 00:00:00'
    ...
    SET @if_as_ard_adl_usei23s_post_insert                    = 'Y'	--- added Jan 18 2022, by Amardeep
	...
	IF @vProdRun = 'Y'
	BEGIN
		SET @vPagerDutyOperator = 'FacAcqPagerTS5'
	...
	BEGIN TRY
		...
		BEGIN
			...
			BEGIN --Start CurCase
				...
				IF @if_as_ard_adl_usei23s_post_insert = 'Y'		--- added Jan 18 2022, by Amardeep
				BEGIN SET @SqlCommand = @SqlCommand + 'exec  [operational].[sproc_facacq_autopre_RemoveTableFromDataCopy] ''as_ard_adl_usei23s'''+CHAR(13)+CHAR(10)+CHAR(13)+CHAR(10) END
				...
				IF @if_as_ard_adl_usei23s_post_insert = 'Y'		 --- added Jan 18 2022, by Amardeep
				BEGIN 
				SET @SqlCommand = @SqlCommand + 'PRINT ''--Post insert for as_ard_adl_usei23s'''+CHAR(13)+CHAR(10)+CHAR(13)+CHAR(10)
				...
				SET @SqlCommand = @SqlCommand + 'INSERT INTO dbo.as_ard_adl_usei23s ( ard_planner_id, std_assess_id, question_usei23, resp_value, source_id, Multi_Fac_Id)'+CHAR(13)+CHAR(10)
				SET @SqlCommand = @SqlCommand + 'SELECT DISTINCT ISNULL(c.dst_id, ard_planner_id), ISNULL(b.dst_id, std_assess_id), [question_usei23], [resp_value], [source_id], '+ @vDstFacId + ''+CHAR(13)+CHAR(10)
				SET @SqlCommand = @SqlCommand + 'FROM ' + @vSourceDBName + '.dbo.as_ard_adl_usei23s a  '+CHAR(13)+CHAR(10)
