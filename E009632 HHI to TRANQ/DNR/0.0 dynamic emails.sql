...
BEGIN
	...
	DECLARE @pmo_number VARCHAR(MAX) = 'E009632'

	DECLARE @src_org_code VARCHAR(MAX) = 'HHI'
	DECLARE @dst_org_code VARCHAR(MAX) = 'TRANQ'

	DECLARE @src_prod VARCHAR(MAX) = ''
	DECLARE @dst_prod VARCHAR(MAX) = ''

	DECLARE @src_test_org_code VARCHAR(MAX) = 'usei435'
	DECLARE @dst_test_org_code VARCHAR(MAX) = 'usei425'

	DECLARE @golive_date VARCHAR(MAX) = 'April 6, 2022'
	DECLARE @golive_day VARCHAR(MAX) = 'Wednesday'
	DECLARE @golive_starttime VARCHAR(MAX) = 'Wednesday, April 6, 6PM'
	DECLare @golive_endtime varchar(max) = '4AM'
	DECLARE @golive_time VARCHAR(MAX) = '6PM to 4AM'
	DECLARE @offline_minutes VARCHAR(MAX) = '5'
	DECLARE @facility_names VARCHAR(MAX) = ''

	DECLARE @all_SRC_facID VARCHAR(MAX) = '20,21'
	DECLARE @all_DST_facID VARCHAR(MAX) = '15,16'
	DECLARE @case_number VARCHAR(MAX) = '' -- netsuite photos & online docs case

	DECLARE @tst VARCHAR(MAX) = '1313'

	DECLARE @srcRegionID varchar(5) = '1,2'
	DECLARE @dstRegionID varchar(5) = '1,2'
	...
	DECLARE @implementers varchar(200) = 'Joyce & Stefani'
	DECLARE @deadline varchar(200) = '***'

	DECLARE @signature varchar(200) = 'Alexander Morcet'
	DECLARE @recepients AS NVARCHAR(max) = 'alexander.morcet@pointclickcare.com'--except this, change this :D
