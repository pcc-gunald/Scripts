...
BEGIN
	...
	DECLARE @pmo_number VARCHAR(MAX) = 'E010805'

	DECLARE @src_org_code VARCHAR(MAX) = 'ACAC'
	DECLARE @dst_org_code VARCHAR(MAX) = 'KHG'

	DECLARE @src_prod VARCHAR(MAX) = '[pccsql-use2-prod-w23-cli0020.4ccb146400ac.database.windows.net].[us_acac]'
	DECLARE @dst_prod VARCHAR(MAX) = '[pccsql-use2-prod-w20-cli0018.3055e0bc69f6.database.windows.net].[us_khg_multi]'

	DECLARE @src_test_org_code VARCHAR(MAX) = 'USEI996'
	DECLARE @dst_test_org_code VARCHAR(MAX) = 'USEI34'

	DECLARE @golive_date VARCHAR(MAX) = 'May, 9, 2022'
	DECLARE @golive_day VARCHAR(MAX) = 'Monday'
	DECLARE @golive_starttime VARCHAR(MAX) = 'Monday, May 9th, 8:00 PM'
	DECLare @golive_endtime varchar(max) = '4:00 AM'
	DECLARE @golive_time VARCHAR(MAX) = '8:00 PM to 4:00 AM'
	DECLARE @offline_minutes VARCHAR(MAX) = '15'
	DECLARE @facility_names VARCHAR(MAX) = 'Acacia Prk Nurshing & Rehab,'

	DECLARE @all_SRC_facID VARCHAR(MAX) = '1'
	DECLARE @all_DST_facID VARCHAR(MAX) = '13'
	DECLARE @case_number VARCHAR(MAX) = '1994666' -- netsuite photos & online docs case

	DECLARE @tst VARCHAR(MAX) = '1366'
	...
	DECLARE @deadline varchar(200) = 'Monday, May 9th'
