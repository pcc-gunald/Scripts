...
BEGIN
	...
	DECLARE @pmo_number VARCHAR(MAX) = '008866'

	DECLARE @src_org_code VARCHAR(MAX) = 'LDMK'
	DECLARE @dst_org_code VARCHAR(MAX) = 'MONA'

	DECLARE @src_prod VARCHAR(MAX) = '[pccsql-use2-prod-w25-cli0010.2d62ac5c7643.database.windows.net].[us_ldmk_multi]'
	DECLARE @dst_prod VARCHAR(MAX) = '[pccsql-use2-prod-w19-cli0009.3055e0bc69f6.database.windows.net].[us_mona_multi]'

	DECLARE @src_test_org_code VARCHAR(MAX) = 'USEI1172'
	DECLARE @dst_test_org_code VARCHAR(MAX) = 'USEI432'

	DECLARE @golive_date VARCHAR(MAX) = 'March, 7, 2022'
	DECLARE @golive_day VARCHAR(MAX) = 'Monday'
	DECLARE @golive_starttime VARCHAR(MAX) = 'Monday, March 7th, 5:00 PM'
	DECLare @golive_endtime varchar(max) = '11:00 PM'
	DECLARE @golive_time VARCHAR(MAX) = '5:00 PM to 11:00 PM'
	...
	DECLARE @deadline varchar(200) = 'Monday, March 7th'
