...
set description='License Nurse Administration Record',short_description='LNAR'
where administration_record_id=1

update  sec_user
set position_id=9091
where position_id=10295
...
set description=LEFT(concat('LDMK-',description),35)
where description not like 'LDMK-%'
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008866_AdminPickList$] a
LEFT JOIN [pccsql-use2-prod-ssv-tscon05.b4ea653240a9.database.windows.net].[test_usei1172].dbo.common_code AS src on src.item_id = a.src_item_id
LEFT JOIN [pccsql-use2-prod-w19-cli0009.3055e0bc69f6.database.windows.net].[us_mona_multi].dbo.common_code AS dst on dst.item_id = a.Map_DstItemId
WHERE 
	(
		...
		AND a.Map_DstItemId NOT IN --remove any dupilicate many to one mappings (Any If_merged not As_is or N)		--
			(
				...
				(
					...
					FROM [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008866_AdminPickList$]
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008866_ResidentIdentifier$] a
...
JOIN [pccsql-use2-prod-w19-cli0009.3055e0bc69f6.database.windows.net].[us_mona_multi].dbo.id_type AS dst on dst.id_type_id = a.map_dst_typeid
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008866_UserDefinedData$] a
...
JOIN [pccsql-use2-prod-w19-cli0009.3055e0bc69f6.database.windows.net].[us_mona_multi].dbo.user_field_types AS dst on dst.field_type_id = a.map_dst_typeid
WHERE  a.srcFieldName IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dst_typeid IS NOT NULL --total count of mapping provided by implementer  
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008866_Clinical_common_code$] a
JOIN [pccsql-use2-prod-ssv-tscon05.b4ea653240a9.database.windows.net].[test_usei1172].dbo.common_code AS src on src.item_id = a.src_item_id
JOIN [pccsql-use2-prod-w19-cli0009.3055e0bc69f6.database.windows.net].[us_mona_multi].dbo.common_code AS dst on dst.item_id = a.Map_DstItemId
WHERE 
	(
		a.pick_list_name IS NOT NULL -- total mapping provided by analyst to implementer							--
		AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer							--
		...
		AND a.Map_DstItemId NOT IN --remove any dupilicate many to one mappings (Any If_merged not As_is or N)		--
			(
				...
				(
					select map_dstitemid 
					FROM [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008866_Clinical_common_code$]
					WHERE pick_list_name IS NOT NULL -- total mapping provided by analyst to implementer	
						AND map_dstitemid IS NOT NULL --total count of mapping provided by implementer		
						AND src_item_id IS NOT NULL --eliminate any incorrect provided mappings	
						AND ISNUMERIC(map_dstitemid) = 1
...
inner join [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008866_Clinical_Advanced$] b 
...
JOIN [pccsql-use2-prod-w19-cli0009.3055e0bc69f6.database.windows.net].[us_mona_multi].dbo.pho_administration_record AS dst on dst.administration_record_id = a.Map_DstItemId
...
JOIN [pccsql-use2-prod-w19-cli0009.3055e0bc69f6.database.windows.net].[us_mona_multi].dbo.pho_order_type AS dst on dst.order_type_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
JOIN [pccsql-use2-prod-w19-cli0009.3055e0bc69f6.database.windows.net].[us_mona_multi].dbo.cr_std_immunization AS dst on dst.std_immunization_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
JOIN [pccsql-use2-prod-w19-cli0009.3055e0bc69f6.database.windows.net].[us_mona_multi].dbo.cp_std_shift AS dst on dst.std_shift_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
--select * from [pccsql-use2-prod-w19-cli0009.3055e0bc69f6.database.windows.net].[us_mona_multi].dbo.mergeTablesmaster where tablename = 'inc_std_pick_list_item'
...
JOIN [pccsql-use2-prod-w19-cli0009.3055e0bc69f6.database.windows.net].[us_mona_multi].dbo.inc_std_pick_list_item AS dst on dst.pick_list_item_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
