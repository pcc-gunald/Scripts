    ...
    (PMONumber,Completed)
    SELECT 
    '008866' as PMONumber, --PMO Number
    0 as Completed

    SELECT pmo_group_id=SCOPE_IDENTITY()

    select * from LoadEIMaster_PMO_Groups where PMONumber like '%008866%'
    ...
    drop table if exists #temp_fac
    create table #temp_fac(caseNo varchar(50),src_id int, dest_id int, id int identity (1,1))
    insert into #temp_fac values('008866',6,4) 
    ...
    'pcc_staging_db008866' as  [STgOrgCode], --staging DB, same for all facilities within one PMO
