...
BEGIN
	...
	DECLARE @pmo_number VARCHAR(MAX) = '008679'

	DECLARE @src_org_code VARCHAR(MAX) = 'WMG'
	DECLARE @dst_org_code VARCHAR(MAX) = 'SYMME'

	DECLARE @src_prod VARCHAR(MAX) = '[pccsql-use2-prod-w24-cli0019.ce22455c967a.database.windows.net].[us_wmg_multi]'
	DECLARE @dst_prod VARCHAR(MAX) = '[pccsql-use2-prod-w19-cli0005.3055e0bc69f6.database.windows.net].[us_symme_multi]'

	DECLARE @src_test_org_code VARCHAR(MAX) = 'usei1129'
	DECLARE @dst_test_org_code VARCHAR(MAX) = 'usei1018'

	DECLARE @golive_date VARCHAR(MAX) = 'February 9, 2022'
	DECLARE @golive_day VARCHAR(MAX) = 'Wednesday'
	DECLARE @golive_starttime VARCHAR(MAX) = 'Wednesday, February 9th, 2PM'
	DECLare @golive_endtime varchar(max) = '4AM'
	DECLARE @golive_time VARCHAR(MAX) = '2PM to 4AM'
	DECLARE @offline_minutes VARCHAR(MAX) = '15'
	DECLARE @facility_names VARCHAR(MAX) = 'TRN Milwaukee LLC, Trinity Village ALF, Trinity Village ILF'

	DECLARE @all_SRC_facID VARCHAR(MAX) = '14, 15, 16'
	DECLARE @all_DST_facID VARCHAR(MAX) = '7, 8, 9'
	DECLARE @case_number VARCHAR(MAX) = '[NOT CREATED YET]' -- netsuite photos & online docs case

	DECLARE @tst VARCHAR(MAX) = '1149'

	DECLARE @srcRegionID varchar(5) = '1,2,3'
	DECLARE @dstRegionID varchar(5) = '1,2,3'
	...
	DECLARE @numDupSecUsers varchar(5) = 'NOT YET!!!'
	DECLARE @implementers varchar(200) = 'FILL IN LATER'
	DECLARE @deadline varchar(200) = 'Wednesday, February 9th'
	...
	DECLARE @recepients AS NVARCHAR(max) = 'dinesh.gunalapan@pointclickcare.com'
