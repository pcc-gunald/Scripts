...
BEGIN
	...
	DECLARE @pmo_number VARCHAR(MAX) = '008255'

	DECLARE @src_org_code VARCHAR(MAX) = 'SSCH'
	DECLARE @dst_org_code VARCHAR(MAX) = 'MEA'

	DECLARE @src_prod VARCHAR(MAX) = '[pccsql-use2-prod-w27-cli0020.851c37dfbe45.database.windows.net].[us_ssch]'
	DECLARE @dst_prod VARCHAR(MAX) = '[pccsql-use2-prod-w19-cli0003.3055e0bc69f6.database.windows.net].[us_mea]'

	DECLARE @src_test_org_code VARCHAR(MAX) = 'USEI964'
	DECLARE @dst_test_org_code VARCHAR(MAX) = 'USEI31'

	DECLARE @golive_date VARCHAR(MAX) = 'March, 14, 2022'
	DECLARE @golive_day VARCHAR(MAX) = 'Monday'
	DECLARE @golive_starttime VARCHAR(MAX) = 'Monday, March 14th, 6:00 PM'
	DECLare @golive_endtime varchar(max) = '4:00 AM'
	DECLARE @golive_time VARCHAR(MAX) = '6:00 PM to 4:00 AM'
	DECLARE @offline_minutes VARCHAR(MAX) = '15'
	DECLARE @facility_names VARCHAR(MAX) = 'Sunnyside Convalescent Hospital'

	DECLARE @all_SRC_facID VARCHAR(MAX) = '1,'
	DECLARE @all_DST_facID VARCHAR(MAX) = '2,'
	DECLARE @case_number VARCHAR(MAX) = '1995861' -- netsuite photos & online docs case

	DECLARE @tst VARCHAR(MAX) = '1111'
	...
	DECLARE @deadline varchar(200) = 'Monday, March 14th'
