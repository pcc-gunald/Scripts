...
;WITH TEMP AS (
SELECT  * 
,COUNT(1) OVER(PARTITION BY item_code, item_description) CNT
,ROW_NUMBER() OVER(PARTITION BY item_code, item_description ORDER BY Item_id) rn
FROM common_code
WHERE deleted='N'
)
--SELECT * 
--,CONCAT(item_description,REPLICATE('_',RN-1))
--FROM TEMP
UPDATE TEMP 
SET item_description=CONCAT(item_description,REPLICATE('_',RN-1))
WHERE 1=1
AND CNT>1
AND RN>1

update [pcc_staging_db008255].[dbo].mergeTablesMaster
set QueryFilter= ' AND ISNULL(reg_id,0)<>2'
...
set  queryfilter= 'AND (
		ext_fac_id IN (
			SELECT ext_fac_id
			FROM [origDB].client_ext_facilities
			WHERE fac_id = [OrigFacId]
			)
		OR ext_fac_id IN (
			SELECT adt_tofrom_loc_id * - 1
			FROM [origDB].census_item
			WHERE fac_id = [OrigFacId]
			)
		OR ext_fac_id IN (
			SELECT pharmacy_id
			FROM [origDB].pho_phys_order
			WHERE fac_id = [OrigFacId]
			)
		OR ext_fac_id IN (
			SELECT reporting_lab_ext_fac_id
			FROM [origDB].result_order_source
			WHERE fac_id = [OrigFacId]
			)
		OR ext_fac_id IN (
		SELECT ext_fac_id
		FROM [origDB].ext_facilities
		WHERE fac_id = [OrigFacId]
		AND hotlist_item=''Y''
		)
		)'
...
set description=LEFT(concat('SSCH-',description),35)
where description not like 'SSCH-%'
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008255_AdminPickList$] a
LEFT JOIN [pccsql-use2-prod-ssv-tscon10.b4ea653240a9.database.windows.net].[test_usei964].dbo.common_code AS src on src.item_id = a.src_item_id
LEFT JOIN [pccsql-use2-prod-w19-cli0003.3055e0bc69f6.database.windows.net].[us_mea].dbo.common_code AS dst on dst.item_id = a.Map_DstItemId
WHERE 1=1
...
AND 	(
		...
		--			FROM [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008255_AdminPickList$]
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008255_ResidentIdentifier$] a
JOIN [pccsql-use2-prod-ssv-tscon10.b4ea653240a9.database.windows.net].[test_usei964].dbo.id_type AS src on src.id_type_id = a.srcIdTypeId
JOIN [pccsql-use2-prod-w19-cli0003.3055e0bc69f6.database.windows.net].[us_mea].dbo.id_type AS dst on dst.id_type_id = a.map_dst_typeid
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008255_UserDefinedData$] a
JOIN [pccsql-use2-prod-ssv-tscon10.b4ea653240a9.database.windows.net].[test_usei964].dbo.user_field_types AS src on src.field_type_id = a.srcFieldTypeId
JOIN [pccsql-use2-prod-w19-cli0003.3055e0bc69f6.database.windows.net].[us_mea].dbo.user_field_types AS dst on dst.field_type_id = a.map_dst_typeid
WHERE  a.srcFieldName IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dst_typeid IS NOT NULL --total count of mapping provided by implementer  
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008255_Clinical_common_code$] a
JOIN [pccsql-use2-prod-ssv-tscon10.b4ea653240a9.database.windows.net].[test_usei964].dbo.common_code AS src on src.item_id = a.src_item_id
JOIN [pccsql-use2-prod-w19-cli0003.3055e0bc69f6.database.windows.net].[us_mea].dbo.common_code AS dst on dst.item_id = a.Map_DstItemId
WHERE 
	(
		a.pick_list_name IS NOT NULL -- total mapping provided by analyst to implementer							--
		AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer							--
		...
		AND a.Map_DstItemId NOT IN --remove any dupilicate many to one mappings (Any If_merged not As_is or N)		--
			(
				...
				(
					select map_dstitemid 
					FROM [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008255_Clinical_common_code$]
					WHERE pick_list_name IS NOT NULL -- total mapping provided by analyst to implementer	
						AND map_dstitemid IS NOT NULL --total count of mapping provided by implementer		
						AND src_item_id IS NOT NULL --eliminate any incorrect provided mappings	
						AND ISNUMERIC(map_dstitemid) = 1
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008255_Clinical_Advanced$] a
inner join [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008255_Clinical_Advanced$] b 
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008255_Clinical_Advanced$] a
JOIN [pccsql-use2-prod-ssv-tscon10.b4ea653240a9.database.windows.net].[test_usei964].dbo.pho_administration_record AS src on src.administration_record_id = a.src_id
JOIN [pccsql-use2-prod-w19-cli0003.3055e0bc69f6.database.windows.net].[us_mea].dbo.pho_administration_record AS dst on dst.administration_record_id = a.Map_DstItemId
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008255_Clinical_Advanced$] a
JOIN [pccsql-use2-prod-ssv-tscon10.b4ea653240a9.database.windows.net].[test_usei964].dbo.pho_order_type AS src on src.order_type_id = a.src_id
JOIN [pccsql-use2-prod-w19-cli0003.3055e0bc69f6.database.windows.net].[us_mea].dbo.pho_order_type AS dst on dst.order_type_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
update src
set retired=dst.retired
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008255_Clinical_Advanced$] a
JOIN [pccsql-use2-prod-ssv-tscon10.b4ea653240a9.database.windows.net].[test_usei964].dbo.pn_type AS src on src.pn_type_id = a.src_id
JOIN [pccsql-use2-prod-w19-cli0003.3055e0bc69f6.database.windows.net].[us_mea].dbo.pn_type AS dst on dst.pn_type_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
--SELECT distinct a.src_id, a.Map_DstItemId, src.description , dst.description, src.short_description, dst.short_description
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008255_Clinical_Advanced$] a
JOIN [pccsql-use2-prod-ssv-tscon10.b4ea653240a9.database.windows.net].[test_usei964].dbo.cr_std_immunization AS src on src.std_immunization_id = a.src_id
JOIN [pccsql-use2-prod-w19-cli0003.3055e0bc69f6.database.windows.net].[us_mea].dbo.cr_std_immunization AS dst on dst.std_immunization_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
--SELECT distinct a.src_id, a.Map_DstItemId, src.description , dst.description
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008255_Clinical_Advanced$] a
JOIN [pccsql-use2-prod-ssv-tscon10.b4ea653240a9.database.windows.net].[test_usei964].dbo.cp_std_shift AS src on src.std_shift_id = a.src_id
JOIN [pccsql-use2-prod-w19-cli0003.3055e0bc69f6.database.windows.net].[us_mea].dbo.cp_std_shift AS dst on dst.std_shift_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
--select * from [pccsql-use2-prod-w19-cli0003.3055e0bc69f6.database.windows.net].[us_mea].dbo.mergeTablesmaster where tablename = 'inc_std_pick_list_item'
...
--SELECT distinct a.src_id, a.Map_DstItemId, src.description , dst.description
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E008255_Clinical_Advanced$] a
JOIN [pccsql-use2-prod-ssv-tscon10.b4ea653240a9.database.windows.net].[test_usei964].dbo.inc_std_pick_list_item AS src on src.pick_list_item_id = a.src_id
JOIN [pccsql-use2-prod-w19-cli0003.3055e0bc69f6.database.windows.net].[us_mea].dbo.inc_std_pick_list_item AS dst on dst.pick_list_item_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
