...
set @CaseNumber = 'EICase59132_P2'--use the root case number, keep the 'EICase'
set @SrcServer = '[pccsql-use2-prod-ssv-tscon10.b4ea653240a9.database.windows.net].[test_usei998]'----change src server & db name
set @DstServer = '[pccsql-use2-prod-ssv-tscon13.b4ea653240a9.database.windows.net].[test_usei44]'----change dst server & db name
...
select
		...
		,CASE
			WHEN dst.created_by like ''%' + @CaseNumber + '%'' THEN ''N''
			WHEN dst.created_by not like ''%' + @CaseNumber + '%'' THEN ''As_is''
			ELSE dst.created_by
		END as ''If_Merged''
--select *
