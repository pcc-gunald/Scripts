...
BEGIN
	...
	DECLARE @pmo_number VARCHAR(MAX) = '59132'

	DECLARE @src_org_code VARCHAR(MAX) = 'JAG'
	DECLARE @dst_org_code VARCHAR(MAX) = 'ELCC'

	DECLARE @src_prod VARCHAR(MAX) = '[pccsql-use2-prod-w25-cli0017.2d62ac5c7643.database.windows.net].[us_jag_multi]'
	DECLARE @dst_prod VARCHAR(MAX) = '[pccsql-use2-prod-w20-cli0007.3055e0bc69f6.database.windows.net].[us_elcc_multi]'

	DECLARE @src_test_org_code VARCHAR(MAX) = 'usei998'
	DECLARE @dst_test_org_code VARCHAR(MAX) = 'usei44'

	DECLARE @golive_date VARCHAR(MAX) = 'March 2, 2022'
	DECLARE @golive_day VARCHAR(MAX) = 'Wednesday'
	DECLARE @golive_starttime VARCHAR(MAX) = 'Wednesday, Macrh 2nd, 12PM'
	DECLare @golive_endtime varchar(max) = 'PM'
	DECLARE @golive_time VARCHAR(MAX) = '12PM to 8PM'
	DECLARE @offline_minutes VARCHAR(MAX) = '15'
	DECLARE @facility_names VARCHAR(MAX) = 'MFF-Mountain Crest '

	DECLARE @all_SRC_facID VARCHAR(MAX) = '8'
	DECLARE @all_DST_facID VARCHAR(MAX) = '34'
	DECLARE @case_number VARCHAR(MAX) = '[NOT CREATED YET]' -- netsuite photos & online docs case

	DECLARE @tst VARCHAR(MAX) = '1807'
	...
	DECLARE @deadline varchar(200) = 'Wednesday, Macrh 2nd'
