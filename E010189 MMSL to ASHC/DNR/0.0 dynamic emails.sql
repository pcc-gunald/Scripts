...
BEGIN
	...
	DECLARE @pmo_number VARCHAR(MAX) = '010189'

	DECLARE @src_org_code VARCHAR(MAX) = 'MMSL'
	DECLARE @dst_org_code VARCHAR(MAX) = 'ASHC'

	DECLARE @src_prod VARCHAR(MAX) = '[pccsql-use2-prod-w26-cli0006.d9c23db323d7.database.windows.net].[us_mmsl_multi]'
	DECLARE @dst_prod VARCHAR(MAX) = '[pccsql-use2-prod-w22-cli0013.4c4638f8e26f.database.windows.net].[us_ashc_multi]'

	DECLARE @src_test_org_code VARCHAR(MAX) = 'USEI963'
	DECLARE @dst_test_org_code VARCHAR(MAX) = 'USEI46'

	DECLARE @golive_date VARCHAR(MAX) = 'March, 23, 2022'
	DECLARE @golive_day VARCHAR(MAX) = 'Wednesday'
	DECLARE @golive_starttime VARCHAR(MAX) = 'Wednesday, March 23rd, 10:00 PM'
	DECLare @golive_endtime varchar(max) = '6:00 AM'
	DECLARE @golive_time VARCHAR(MAX) = '10:00 PM to 6:00 AM'
	...
	DECLARE @all_SRC_facID VARCHAR(MAX) = '11'
	DECLARE @all_DST_facID VARCHAR(MAX) = '35'
	DECLARE @case_number VARCHAR(MAX) = '1994666' -- netsuite photos & online docs case

	DECLARE @tst VARCHAR(MAX) = '1325'
	...
	DECLARE @deadline varchar(200) = 'Wednesday, March 23rd'
