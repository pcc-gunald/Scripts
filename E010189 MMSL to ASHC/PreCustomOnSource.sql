...
set description=LEFT(concat('MMSL-',description),35)
where description not like 'MMSL-%'
and brand_id is NULL

UPDATE pho_std_order
SET template_description=CONCAT(template_description,'_')
WHERE template_description='Fleet Enema'



update sec_role 
set description = 'MMSL-' + description 
--select * 
from sec_role 
where (system_field <> 'Y' or system_field is null)
and description not like 'MMSL%'
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E010189_AdminPickList$] a
...
LEFT JOIN [pccsql-use2-prod-w22-cli0013.4c4638f8e26f.database.windows.net].[us_ashc_multi].dbo.common_code AS dst on dst.item_id = a.Map_DstItemId
WHERE 
	(
		...
		AND a.Map_DstItemId NOT IN --remove any dupilicate many to one mappings (Any If_merged not As_is or N)		--
			(
				...
				(
					...
					FROM [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E010189_AdminPickList$]
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E010189_ResidentIdentifier$] a
...
JOIN [pccsql-use2-prod-w22-cli0013.4c4638f8e26f.database.windows.net].[us_ashc_multi].dbo.id_type AS dst on dst.id_type_id = a.map_dst_typeid
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E010189_UserDefinedData$] a
...
JOIN [pccsql-use2-prod-w22-cli0013.4c4638f8e26f.database.windows.net].[us_ashc_multi].dbo.user_field_types AS dst on dst.field_type_id = a.map_dst_typeid
WHERE  a.srcFieldName IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dst_typeid IS NOT NULL --total count of mapping provided by implementer  
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E010189_Clinical_common_code$] a
...
JOIN [pccsql-use2-prod-w22-cli0013.4c4638f8e26f.database.windows.net].[us_ashc_multi].dbo.common_code AS dst on dst.item_id = a.Map_DstItemId
WHERE 
	(
		a.pick_list_name IS NOT NULL -- total mapping provided by analyst to implementer							--
		AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer							--
		...
		AND a.Map_DstItemId NOT IN --remove any dupilicate many to one mappings (Any If_merged not As_is or N)		--
			(
				...
				(
					select map_dstitemid 
					FROM [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E010189_Clinical_common_code$]
					WHERE pick_list_name IS NOT NULL -- total mapping provided by analyst to implementer	
						AND map_dstitemid IS NOT NULL --total count of mapping provided by implementer		
						AND src_item_id IS NOT NULL --eliminate any incorrect provided mappings	
						AND ISNUMERIC(map_dstitemid) = 1
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E010189_Clinical_Advanced$] a
inner join [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E010189_Clinical_Advanced$] b 
...
JOIN [pccsql-use2-prod-w22-cli0013.4c4638f8e26f.database.windows.net].[us_ashc_multi].dbo.pho_administration_record AS dst on dst.administration_record_id = a.Map_DstItemId
...
JOIN [pccsql-use2-prod-w22-cli0013.4c4638f8e26f.database.windows.net].[us_ashc_multi].dbo.pho_order_type AS dst on dst.order_type_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
and a.pick_list_name = 'Order Types' 


--SELECT a.src_desc,b.dst_desc FROM  [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E010189_Clinical_Advanced$] A
--INNER JOIN  [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E010189_Clinical_Advanced$] B ON B.dst_id=A.Map_DstItemId AND B.pick_list_name=A.pick_list_name
...
JOIN [pccsql-use2-prod-w22-cli0013.4c4638f8e26f.database.windows.net].[us_ashc_multi].dbo.pn_type AS dst on dst.pn_type_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
JOIN [pccsql-use2-prod-w22-cli0013.4c4638f8e26f.database.windows.net].[us_ashc_multi].dbo.cr_std_immunization AS dst on dst.std_immunization_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
JOIN [pccsql-use2-prod-w22-cli0013.4c4638f8e26f.database.windows.net].[us_ashc_multi].dbo.cp_std_shift AS dst on dst.std_shift_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
--select * from [pccsql-use2-prod-w22-cli0013.4c4638f8e26f.database.windows.net].[us_ashc_multi].dbo.mergeTablesmaster where tablename = 'inc_std_pick_list_item'
...
JOIN [pccsql-use2-prod-w22-cli0013.4c4638f8e26f.database.windows.net].[us_ashc_multi].dbo.inc_std_pick_list_item AS dst on dst.pick_list_item_id = a.Map_DstItemId
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
