...
print  CHAR(13) + 'Starting to run script : GAP - cp_sec_user_audit - rescoping for fac R_SRCFACID7' 
...
where client_id in (select client_id from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].clients with (nolock) where fac_id = R_SRCFACID7)) and fac_id <> R_SRCFACID7)
and fac_id <> R_SRCFACID7 
...
where b.fac_id = R_SRCFACID7 and s.fac_id <> R_SRCFACID7 
...
(select created_by_audit_id from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].pho_std_order_set with (nolock) where  fac_id  in (-1,R_SRCFACID7)
union 
select revision_by_audit_id from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].pho_std_order_set with (nolock) where  fac_id  in (-1,R_SRCFACID7)
union 
select published_by_audit_id from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].pho_std_order_set with (nolock) where  fac_id  in (-1,R_SRCFACID7)
union 
select retired_by_audit_id from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].pho_std_order_set with (nolock) where fac_id  in (-1,R_SRCFACID7)
union 
select reactivated_by_audit_id from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].pho_std_order_set with (nolock) where  fac_id  in (-1,R_SRCFACID7))
and fac_id <> R_SRCFACID7)
and fac_id <> R_SRCFACID7 
...
where userid in 
(select userid from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].cp_sec_user_audit with (nolock) where cp_sec_user_audit_id in 
(select created_by_audit_id from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].pho_std_order with (nolock) where  fac_id  in (-1,R_SRCFACID7)
union 
select revision_by_audit_id from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].pho_std_order with (nolock) where  fac_id  in (-1,R_SRCFACID7)
union 
select published_by_audit_id from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].pho_std_order with (nolock) where  fac_id  in (-1,R_SRCFACID7)
union 
select retired_by_audit_id from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].pho_std_order with (nolock) where  fac_id  in (-1,R_SRCFACID7)
union 
select reactivated_by_audit_id from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].pho_std_order with (nolock) where  fac_id  in (-1,R_SRCFACID7))
and fac_id <> R_SRCFACID7)
and fac_id <> R_SRCFACID7 
...
where b.client_id in (select client_id from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].clients with (nolock) where fac_id = R_SRCFACID7))  
...
where fac_id = R_SRCFACID7)) and  fac_id <> R_SRCFACID7)
and fac_id <> R_SRCFACID7
...
where b.fac_id = R_SRCFACID7 and s.fac_id <> R_SRCFACID7 and (facility_id= R_SRCFACID7 or admin_user_type ='E') 
...
where cp_sec_user_audit_id in  (select  sec_user_audit_id from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].immunization_strikeout with (nolock)
where immunization_id in (select  immunization_id  from   [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].cr_client_immunization with (nolock)
where fac_id in (R_SRCFACID7)))
and  fac_id not in (R_SRCFACID7))
and fac_id not in (R_SRCFACID7) 
...
where client_id in (select client_id from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].clients with (nolock) where fac_id in (R_SRCFACID7)))
and fac_id not in (R_SRCFACID7))
and fac_id not in (R_SRCFACID7)
...
where client_diagnosis_id in (select client_diagnosis_id from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].diagnosis with (nolock) where fac_id in (R_SRCFACID7)))
and fac_id not in (R_SRCFACID7))
and fac_id not in (R_SRCFACID7)
...
union 
	...
	JOIN [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].pho_admin_order_audit b with (nolock) on a.admin_order_id = b.admin_order_id 
	JOIN [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].pho_phys_order c with (nolock) on b.phys_order_id = c.phys_order_id
	WHERE c.fac_id = R_SRCFACID7) and fac_id <> R_SRCFACID7)
and fac_id <> R_SRCFACID7
...
WHERE userid IN (
		...
		WHERE cp_sec_user_audit_id IN (
			...
			AND fac_id <> R_SRCFACID7) AND fac_id <> R_SRCFACID7
...
WHERE phys_order_id IN (SELECT distinct phys_order_id FROM [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].pho_phys_order with (nolock) WHERE fac_id = R_SRCFACID7))
AND fac_id <> R_SRCFACID7)
and fac_id <> R_SRCFACID7
...
where custom_drug_id in (select custom_drug_id from [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.cr_cust_med with (nolock) where fac_id in (-1,R_SRCFACID7))
union
...
where custom_drug_id in (select custom_drug_id from [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.cr_cust_med with (nolock) where fac_id in (-1,R_SRCFACID7)))
and fac_id not in (-1,R_SRCFACID7)
...
if OBJECT_ID ('pcc_temp_storage.dbo._bkp_PMO59277_CaseR_CASENUMBER7_sec_user_import_pre_scoping_fac_R_SRCFACID7_pregolive','U') is not null
drop table pcc_temp_storage.dbo._bkp_PMO59277_CaseR_CASENUMBER7_sec_user_import_pre_scoping_fac_R_SRCFACID7_pregolive

print  CHAR(13) + 'the below should be 0' 

select *  into pcc_temp_storage.dbo._bkp_PMO59277_CaseR_CASENUMBER7_sec_user_import_pre_scoping_fac_R_SRCFACID7_pregolive
from #temp where userid not in (select src_id from test_usei1214.dbo.EICaseR_CASENUMBER7sec_user) and loginname not like '%pcc-%' 

print  CHAR(13) + 'the above should be 0' 

print  CHAR(13) + 'Ended running : GAP cp_sec_user_audit - rescoping for fac R_SRCFACID7' 

print  CHAR(13) + 'Starting to run script : GAP import GAP of security users for fac R_SRCFACID7' 
...
join [pccsql-use2-prod-ssv-tscon14.b4ea653240a9.database.windows.net].test_usei1214.dbo.eicaseR_CASENUMBER7sec_user map
on a.userid = map.src_id
where a.userid in 
(select src_id from [pccsql-use2-prod-ssv-tscon14.b4ea653240a9.database.windows.net].test_usei1214.dbo.eicaseR_CASENUMBER7sec_user 
...
if OBJECT_ID ('pcc_temp_storage.dbo.EIcaseR_CASENUMBER7_PGHC_sec_user_sameloginname_pregolive','U') is not null
DROP TABLE pcc_temp_storage.dbo.EIcaseR_CASENUMBER7_PGHC_sec_user_sameloginname_pregolive

if OBJECT_ID ('pcc_temp_storage.dbo.EIcaseR_CASENUMBER7_PGHC_sec_user_samelogin_different_name_pregolive','U') is not null
DROP TABLE pcc_temp_storage.dbo.EIcaseR_CASENUMBER7_PGHC_sec_user_samelogin_different_name_pregolive

if OBJECT_ID ('pcc_temp_storage.dbo.EIcaseR_CASENUMBER7_PGHC_sec_user_samelogin_same_name_pregolive','U') is not null
DROP TABLE pcc_temp_storage.dbo.EIcaseR_CASENUMBER7_PGHC_sec_user_samelogin_same_name_pregolive

if OBJECT_ID ('dbo.EIcaseR_CASENUMBER7sec_user_pregolive','U') is not null
DROP TABLE dbo.EIcaseR_CASENUMBER7sec_user_pregolive
...
select  a.* into pcc_temp_storage.dbo.EIcaseR_CASENUMBER7_PGHC_sec_user_sameloginname_pregolive
from [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.sec_user a 
join  dbo.sec_user b 
on a.loginname = b.loginname
where a.fac_id = R_SRCFACID7
and a.loginname not like '%pcc-%' 
and a.loginname  not like '%wescom%' 
and a.userid not in (select src_id from EICaseR_CASENUMBER7sec_user)

insert into pcc_temp_storage.dbo.EIcaseR_CASENUMBER7_PGHC_sec_user_sameloginname_pregolive
select a.*
from [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.sec_user a
join  dbo.sec_user b
on a.loginname = b.loginname
where a.userid in (select distinct userid from pcc_temp_storage.dbo._bkp_PMO59277_CaseR_CASENUMBER7_sec_user_import_pre_scoping_fac_R_SRCFACID7_pregolive) --userid from scoping

declare @MaxSecUserid int, @Rowcount int, @facid int

select identity(int,0,1) as row_id, src.userid as src_id, NULL as dst_id
into dbo.EIcaseR_CASENUMBER7sec_user_pregolive 
from [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.sec_user src 
where fac_id = R_SRCFACID7 and loginname not like '%pcc-%' and loginname not like '%wescom%' 
and userid not in (select src_id from EICaseR_CASENUMBER7sec_user)
or userid in (select distinct userid from pcc_temp_storage.dbo._bkp_PMO59277_CaseR_CASENUMBER7_sec_user_import_pre_scoping_fac_R_SRCFACID7_pregolive)

set @Rowcount=@@rowcount
exec get_next_primary_key 'sec_user','userid',@MaxSecUserid output , @Rowcount

update  dbo.EIcaseR_CASENUMBER7sec_user_pregolive 
...
SELECT map.dst_id
	...
	,default_admin_view,sso_only_user
FROM [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.sec_user src
JOIN dbo.EIcaseR_CASENUMBER7sec_user_pregolive map ON src.userid = map.src_id
WHERE src.userid NOT IN (SELECT userid FROM pcc_temp_storage.dbo.EIcaseR_CASENUMBER7_PGHC_sec_user_sameloginname_pregolive)

INSERT INTO dbo.sec_user_facility (userid,facility_id,access_level)
SELECT DISTINCT a.dst_id
	,R_DSTFACID7   
	,src.access_level
FROM dbo.EIcaseR_CASENUMBER7sec_user_pregolive a join [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.sec_user_facility src
on a.src_id = src.userid
WHERE src.userid NOT IN (SELECT userid FROM pcc_temp_storage.dbo.EIcaseR_CASENUMBER7_PGHC_sec_user_sameloginname_pregolive) and src.facility_id = R_SRCFACID7

--==================================================================================
--Step 2. Import users with same loginname but different long_username adding suffix
--==================================================================================

select  a.* into pcc_temp_storage.dbo.EIcaseR_CASENUMBER7_PGHC_sec_user_samelogin_different_name_pregolive
from [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.sec_user a
join  dbo.sec_user b
on a.loginname = b.loginname
where a.fac_id = R_SRCFACID7 
...
insert into pcc_temp_storage.dbo.EIcaseR_CASENUMBER7_PGHC_sec_user_samelogin_different_name_pregolive
select a.*
from [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.sec_user a
join  dbo.sec_user b
on a.loginname = b.loginname
where a.userid in (select distinct userid from pcc_temp_storage.dbo._bkp_PMO59277_CaseR_CASENUMBER7_sec_user_import_pre_scoping_fac_R_SRCFACID7_pregolive)--94 --userid from scoping
and a.long_username <> b.long_username
...
FROM  [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.sec_user src 
JOIN dbo.EIcaseR_CASENUMBER7sec_user_pregolive map ON src.userid = map.src_id
and src_id IN (select userid from pcc_temp_storage.dbo.EIcaseR_CASENUMBER7_PGHC_sec_user_samelogin_different_name_pregolive)
...
FROM dbo.EIcaseR_CASENUMBER7sec_user_pregolive a join [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.sec_user_facility src
on a.src_id = src.userid
JOIN pcc_temp_storage.dbo.EIcaseR_CASENUMBER7_PGHC_sec_user_samelogin_different_name_pregolive t on src.userid = t.userid
WHERE src.userid IN (SELECT userid FROM pcc_temp_storage.dbo.EIcaseR_CASENUMBER7_PGHC_sec_user_samelogin_different_name_pregolive) 
and src.facility_id = R_SRCFACID7

--================================
--Step 3. Update table for testing
--================================
ALTER TABLE dbo.EIcaseR_CASENUMBER7sec_user_pregolive
add corporate char
GO
...
select  a.* into pcc_temp_storage.dbo.EIcaseR_CASENUMBER7_PGHC_sec_user_samelogin_same_name_pregolive
from [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.sec_user a
join  dbo.sec_user b
on a.loginname = b.loginname
where a.fac_id = R_SRCFACID7 
and a.loginname not like '%pcc-%' and a.loginname  not like '%wescom%'
and a.long_username = b.long_username 
and b.created_by not like '%EICaseR_CASENUMBER7%'
and a.userid not in (select src_id from EICaseR_CASENUMBER7sec_user)

insert  into pcc_temp_storage.dbo.EIcaseR_CASENUMBER7_PGHC_sec_user_samelogin_same_name_pregolive
select a.*
from [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.sec_user a
join  dbo.sec_user b
on a.loginname = b.loginname
where a.userid in (select distinct userid from pcc_temp_storage.dbo._bkp_PMO59277_CaseR_CASENUMBER7_sec_user_import_pre_scoping_fac_R_SRCFACID7_pregolive)
and a.long_username = b.long_username 
and b.created_by not like '%EICaseR_CASENUMBER7%' 

update map
set dst_id = ur.userid, corporate = 'Y'
FROM dbo.EIcaseR_CASENUMBER7sec_user_pregolive map 
JOIN pcc_temp_storage.dbo.EIcaseR_CASENUMBER7_PGHC_sec_user_samelogin_same_name_pregolive ln ON ln.userid = map.src_id
...
JOIN dbo.EIcaseR_CASENUMBER7sec_user_pregolive b ON b.src_id = src.userid 
where b.corporate = 'N' 

SET IDENTITY_INSERT EIcaseR_CASENUMBER7sec_user ON 

insert into EIcaseR_CASENUMBER7sec_user (row_id,src_id,dst_id,corporate)
select row_id, src_id,dst_id,corporate from EIcaseR_CASENUMBER7sec_user_pregolive
where src_id not in (select src_id from EIcaseR_CASENUMBER7sec_user) 

SET IDENTITY_INSERT EIcaseR_CASENUMBER7sec_user OFF
...
----select *
--FROM dbo.EICaseR_CASENUMBER7sec_user map 
--join pcc_temp_storage.dbo.EICaseR_CASENUMBER7_PGHC_sec_user_samelogin_different_name ln on map.src_id = ln.userid
--JOIN sec_user ur ON ln.loginname+ 'PGHC' = ur.loginname--3
--where ur.created_by like 'EICaseR_CASENUMBER7' --prev facility

print CHAR(13) + 'performing final checks'

print CHAR(13) + 'all results below should be 0'

--select * from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].sec_user where fac_id in (R_SRCFACID7) and created_date >= '????-??-??'
--and loginname not like '%pcc-%'

select distinct noted_by,'fac R_SRCFACID7'  from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].pho_admin_order a
join [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].pho_phys_order b
on a.phys_order_id = b.phys_order_id 
where b.fac_id in (R_SRCFACID7)
and noted_by not in (select src_id from [pccsql-use2-prod-ssv-tscon14.b4ea653240a9.database.windows.net].test_usei1214.dbo.EIcaseR_CASENUMBER7sec_user)

select distinct strikeout_by_id,'fac R_SRCFACID7'  from [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].inc_incident 
where fac_id in (R_SRCFACID7)
and strikeout_by_id not in (select src_id from [pccsql-use2-prod-ssv-tscon14.b4ea653240a9.database.windows.net].test_usei1214.dbo.EIcaseR_CASENUMBER7sec_user)

select * from [pccsql-use2-prod-ssv-tscon14.b4ea653240a9.database.windows.net].test_usei1214.dbo.EIcaseR_CASENUMBER7sec_user
where dst_id not in (select userid from [pccsql-use2-prod-ssv-tscon14.b4ea653240a9.database.windows.net].test_usei1214.dbo.sec_user)
and dst_id not in (-10000,-998,1)
...
and (created_by like '%EIcaseR_CASENUMBER7%' or userid in (select dst_id from EIcaseR_CASENUMBER7sec_user where corporate = 'N'))
and admin_user_type is NULL

print CHAR(13) + 'all results above should be 0'

print  CHAR(13) + 'Ended running script : import GAP of security users for fac R_SRCFACID7' 
