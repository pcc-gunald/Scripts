...
join EICaseR_CASENUMBER9as_assessment map1 on map1.src_id = src.assess_id ------mapping table
join EICaseR_CASENUMBER9as_assessment map2 on map2.src_id = src.first_assess_id  ------mapping table
...
from sec_user dst
join EICaseR_CASENUMBER9sec_user m 
...
where src.userid = f.userid and f.facility_id = R_SRCFACID9)
and src.enabled = 'Y'
...
JOIN [dbo].EICaseR_CASENUMBER9common_code m ON s.common_code_item_id = m.src_id
JOIN [dbo].common_code d ON m.dst_id = d.item_id
WHERE m.dst_id NOT IN (SELECT common_code_item_id FROM [dbo].[common_code_standard_contact_type_mapping]) AND d.fac_id = R_DSTFACID9 
...
--inner join dbo.EICaseR_CASENUMBER9census_item map--
...
--where dst.fac_id in (R_DSTFACID9)--
...
--where dst.fac_id in (R_DSTFACID9)--dst fac_id
--and alp.payer_id not in (select payer_id from ar_payers where fac_id in (R_DSTFACID9))--dst fac_id
--and map_dstpayerid is not null 
...
--WHERE dst.fac_id IN (R_DSTFACID9) --dst
...
--inner join [dbo].[EICaseR_CASENUMBER9room_date_range] map--
...
--WHERE dst.room_id in  (select  room_id from room where fac_id = R_DSTFACID9)
...
join test_usei1214.dbo.EICaseR_CASENUMBER9sec_user umap on dst.userid = umap.dst_id
join [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.sec_user src on umap.src_id = src.userid
join test_usei1214.dbo.EICaseR_CASENUMBER9contact cmap on src.staff_id = cmap.src_id
...
where user_id not in (select dst_id from EICaseR_CASENUMBER9sec_user) 
and created_by like '%EICaseR_CASENUMBER9%'
...
join EICaseR_CASENUMBER9sec_user map on dst.user_id = map.src_id
...
IF OBJECT_ID('pcc_temp_storage.dbo._bkp_sec_user_snapshot_CaseR_CASENUMBER9', 'U') IS NOT NULL 
	DROP TABLE pcc_temp_storage.dbo._bkp_sec_user_snapshot_CaseR_CASENUMBER9
SELECT * into pcc_temp_storage.dbo._bkp_sec_user_snapshot_CaseR_CASENUMBER9 from sec_user

update sec_user
set email = '', alt_email = ''
where ((email is not null and email <> '') or (alt_email is not null and alt_email <> ''))
and userid in (select dst_id from EICaseR_CASENUMBER9sec_user where corporate = 'N')
...
left join [dbo].EICaseR_CASENUMBER9mpi_history m
on s.mpi_history_id = m.src_id
left join [dbo].mpi_history d
on d.mpi_history_id = m.dst_id and d.fac_id = R_DSTFACID9
where d.mpi_history_id is null 
and s.fac_id = R_SRCFACID9
...
where a.facility_id = R_SRCFACID9))
...
SELECT 
	DISTINCT b.dst_id,ISNULL(EICaseR_CASENUMBER91.dst_id, mpi_id),copy_fac.dst_id,ISNULL(EICaseR_CASENUMBER910.dst_id, user_id),[created_by],[created_date],[revision_by],[revision_date]
	,[title],[first_name],[last_name],[middle_name],[maiden_name],[date_of_birth],[place_of_birth],ISNULL(EICaseR_CASENUMBER94.dst_id, marital_status_id),[sex],[address1],[address2]
	,[address3],[city]	,[county_id],[prov_state],[postal_zip_code],ISNULL(EICaseR_CASENUMBER99.dst_id, country_id),[deceased_date],ISNULL(EICaseR_CASENUMBER95.dst_id, religion_id)
	,ISNULL(EICaseR_CASENUMBER96.dst_id, race_id),ISNULL(EICaseR_CASENUMBER92.dst_id, primary_lang_id),ISNULL(EICaseR_CASENUMBER93.dst_id, secondary_lang_id)
	,ISNULL(EICaseR_CASENUMBER98.dst_id, citizenship_id),[allergy],[height],[ibw_range]	,[veteran_number],[public_trustee_number],ISNULL(EICaseR_CASENUMBER97.dst_id, education_id),[occupations]
...
JOIN test_usei1214.[dbo].EICaseR_CASENUMBER9facility copy_fac ON copy_fac.src_id = a.fac_id OR copy_fac.src_id = R_SRCFACID9
JOIN test_usei1214.[dbo].EICaseR_CASENUMBER9mpi EICaseR_CASENUMBER91 ON EICaseR_CASENUMBER91.src_id = a.mpi_id
LEFT JOIN test_usei1214.[dbo].EICaseR_CASENUMBER9common_code EICaseR_CASENUMBER92 ON EICaseR_CASENUMBER92.src_id = a.primary_lang_id
LEFT JOIN test_usei1214.[dbo].EICaseR_CASENUMBER9common_code EICaseR_CASENUMBER93 ON EICaseR_CASENUMBER93.src_id = a.secondary_lang_id
LEFT JOIN test_usei1214.[dbo].EICaseR_CASENUMBER9common_code EICaseR_CASENUMBER94 ON EICaseR_CASENUMBER94.src_id = a.marital_status_id
LEFT JOIN test_usei1214.[dbo].EICaseR_CASENUMBER9common_code EICaseR_CASENUMBER95 ON EICaseR_CASENUMBER95.src_id = a.religion_id
LEFT JOIN test_usei1214.[dbo].EICaseR_CASENUMBER9common_code EICaseR_CASENUMBER96 ON EICaseR_CASENUMBER96.src_id = a.race_id
LEFT JOIN test_usei1214.[dbo].EICaseR_CASENUMBER9common_code EICaseR_CASENUMBER97 ON EICaseR_CASENUMBER97.src_id = a.education_id
LEFT JOIN test_usei1214.[dbo].EICaseR_CASENUMBER9common_code EICaseR_CASENUMBER98 ON EICaseR_CASENUMBER98.src_id = a.citizenship_id
LEFT JOIN test_usei1214.[dbo].EICaseR_CASENUMBER9common_code EICaseR_CASENUMBER99 ON EICaseR_CASENUMBER99.src_id = a.country_id
LEFT JOIN test_usei1214.[dbo].EICaseR_CASENUMBER9sec_user EICaseR_CASENUMBER910 ON EICaseR_CASENUMBER910.src_id = a.user_id ,test_usei1214.[dbo].EICaseR_CASENUMBER9mpi_history b
WHERE a.mpi_history_id <> - 1
	AND a.fac_id IN (R_SRCFACID9,- 1)
...
if OBJECT_ID ('pcc_temp_storage.dbo._bkp_diagnosis_PGHC_post_EICaseR_CASENUMBER9diagnosis_codes_post','U') is not null
	drop table pcc_temp_storage.dbo._bkp_diagnosis_PGHC_post_EICaseR_CASENUMBER9diagnosis_codes_post

if OBJECT_ID ('dbo.EICaseR_CASENUMBER9diagnosis_codes_post','U') is not null
	drop table dbo.EICaseR_CASENUMBER9diagnosis_codes_post

--1. Generate post mapping table, populating affected src diagnosis_id
select distinct map.* 
into EICaseR_CASENUMBER9diagnosis_codes_post --update Case #
--select distinct map.*
--select src.*
from EICaseR_CASENUMBER9diagnosis_codes map
...
update EICaseR_CASENUMBER9diagnosis_codes_post set corporate = 'N'
...
join EICaseR_CASENUMBER9diagnosis_codes_post map2 on map2.src_id = src.diagnosis_id
where src.diagnosis_id in (select distinct src_id from EICaseR_CASENUMBER9diagnosis_codes_post) 
...
from diagnosis dst
join EICaseR_CASENUMBER9diagnosis mapd on dst.client_diagnosis_id = mapd.dst_id
join EICaseR_CASENUMBER9diagnosis_codes map on map.dst_id = dst.diagnosis_id
join EICaseR_CASENUMBER9diagnosis_codes_post map2 on map2.src_id = map.src_id
...
(
select src_id from EICaseR_CASENUMBER9diagnosis_codes 
	where src_id not in (select src_id from EICaseR_CASENUMBER9diagnosis_codes_post where corporate = 'Y')
	and dst_id NOT IN (SELECT diagnosis_id FROM dbo.diagnosis_codes_clinical_category)
union
select src_id from EICaseR_CASENUMBER9diagnosis_codes_post where corporate = 'Y'
	and dst_id NOT IN (SELECT diagnosis_id FROM dbo.diagnosis_codes_clinical_category)
	)
...
where fac_id = R_DSTFACID9 
and file_id is not null 
and file_id in (select file_metadata_id from [sqluspaw29cli01.pccprod.local].us_sava_multi.dbo.file_metadata)
and file_id not in (select dst_id from EICaseR_CASENUMBER9file_metadata) 
and created_by = 'EICaseR_CASENUMBER9'
...
WHERE fac_id = R_DSTFACID9 
    ...
    AND file_metadata_id IN (SELECT dst_id FROM EICaseR_CASENUMBER9file_metadata) 
...
WHERE audit_id IN (SELECT src_id FROM EICaseR_CASENUMBER9pho_phys_order_audit)--are copied --mapping table
AND phys_order_id NOT IN (SELECT phys_order_id FROM [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.pho_phys_order with (nolock) WHERE fac_id = R_SRCFACID9) --src fac_id

select * into #stru_tempb from pho_phys_order_audit with (nolock)
where fac_id = R_DSTFACID9 --(253548 row(s) affected) --dst fac_id
and client_id in (select dst_id from EICaseR_CASENUMBER9Clients with (nolock))----mapping table

SELECT dst_id into #stru_tempdstid FROM EICaseR_CASENUMBER9pho_phys_order_audit with (nolock)--copied over by E/I --mapping table
...
CREATE TABLE EICaseR_CASENUMBER9StruckOUT (row_id INT IDENTITY,src_id BIGINT,dst_id BIGINT)

INSERT INTO EICaseR_CASENUMBER9StruckOUT (src_id)
SELECT distinct phys_order_id FROM #stru_temps-- Jimmy  Added  Distinct

DECLARE @Maxphys_order_id INT,@Rowcount0 INT,@facid0 INT;
SET @Rowcount0 = (SELECT count(1) from EICaseR_CASENUMBER9StruckOUT)
EXEC get_next_primary_key 'pho_phys_order ' ,'phys_order_id',@Maxphys_order_id OUTPUT,@Rowcount0

UPDATE EICaseR_CASENUMBER9StruckOUT
...
JOIN EICaseR_CASENUMBER9StruckOUT b
ON a.phys_order_id = b.src_id
where 
a.client_id in (select dst_id from EICaseR_CASENUMBER9Clients )--
and a.fac_id = R_DSTFACID9 --dst fac_id
...
from EICaseR_CASENUMBER9StruckOUT
...
and b.fac_id = R_SRCFACID9 --src fac_id

CREATE TABLE EICaseR_CASENUMBER9pho_admin_order_audit_post (row_id INT IDENTITY,src_id BIGINT,dst_id BIGINT)

INSERT INTO EICaseR_CASENUMBER9pho_admin_order_audit_post (src_id)
SELECT 
distinct audit_id FROM #stru_temp

DECLARE @Maxpho_admin_order_audit INT,@Rowcount1 INT,@facid1 INT;
SET @Rowcount1 = (SELECT count(1) from EICaseR_CASENUMBER9pho_admin_order_audit_post)
EXEC get_next_primary_key 'pho_admin_order_audit ' ,'audit_id',@Maxpho_admin_order_audit OUTPUT,@Rowcount1

UPDATE EICaseR_CASENUMBER9pho_admin_order_audit_post
...
CREATE TABLE EICaseR_CASENUMBER9pho_admin_order_post (row_id INT IDENTITY,src_id BIGINT,dst_id BIGINT)

INSERT INTO EICaseR_CASENUMBER9pho_admin_order_post (src_id)
SELECT 
distinct admin_order_id FROM #stru_temp2

DECLARE @Maxpho_admin_order INT,@Rowcount2 INT,@facid2 INT;
SET @Rowcount2 = (SELECT count(1) from EICaseR_CASENUMBER9pho_admin_order_post)
EXEC get_next_primary_key 'pho_admin_order ' ,'admin_order_id',@Maxpho_admin_order OUTPUT,@Rowcount2

UPDATE EICaseR_CASENUMBER9pho_admin_order_post
...
SELECT DISTINCT b.dst_id
	,a.[event_type]
	,ISNULL(EICaseR_CASENUMBER91.dst_id, a.admin_order_id)
	,ISNULL(EICaseR_CASENUMBER93.dst_id, a.phys_order_id)
	,a.[effective_date]
	,isnull(a.ineffective_date, getdate())
	,ISNULL(EICaseR_CASENUMBER92.dst_id, a.physician_id)
	,a.[created_by]
	,a.[created_date]
	,a.[revision_by]
	,a.[revision_date]
	...
	,a.[deleted_date]
	,a.[reason]
	,ISNULL(EICaseR_CASENUMBER94.dst_id, a.noted_by)
	,a.[strikeout_by]
	,a.[strikeout_date]
...
LEFT JOIN [dbo].EICaseR_CASENUMBER9pho_admin_order_post EICaseR_CASENUMBER91 ON EICaseR_CASENUMBER91.src_id = a.admin_order_id
LEFT JOIN [dbo].EICaseR_CASENUMBER9contact EICaseR_CASENUMBER92 ON EICaseR_CASENUMBER92.src_id = a.physician_id
JOIN [dbo].EICaseR_CASENUMBER9StruckOUT EICaseR_CASENUMBER93 ON EICaseR_CASENUMBER93.src_id = a.phys_order_id --S/O post mapping table
LEFT JOIN [dbo].EICaseR_CASENUMBER9sec_user EICaseR_CASENUMBER94 ON EICaseR_CASENUMBER94.src_id = a.noted_by --nullable
	...
	,[dbo].EICaseR_CASENUMBER9pho_admin_order_audit_post b
WHERE 
	a.audit_id <> - 1
	AND a.audit_id = b.src_id
	AND pa.client_id in (select src_id from EICaseR_CASENUMBER9clients) 
...
and b.fac_id = R_SRCFACID9 --src fac_id
and a.fac_id = R_SRCFACID9 --src fac_id
and b.client_id in (select src_id from EICaseR_CASENUMBER9clients)
union
SELECT distinct a.audit_id
--INTO #stru_tempa3
--select distinct a.*
...
CREATE TABLE EICaseR_CASENUMBER9pho_related_order_audit_post (row_id INT IDENTITY,src_id BIGINT,dst_id BIGINT)

INSERT INTO EICaseR_CASENUMBER9pho_related_order_audit_post (src_id)
SELECT 
distinct audit_id FROM #stru_tempa


DECLARE @Maxpho_related_order_audit INT,@Rowcount3 INT,@facid3 INT;
SET @Rowcount3 = (SELECT count(1) from EICaseR_CASENUMBER9pho_related_order_audit_post)
EXEC get_next_primary_key 'pho_related_order_audit ' ,'audit_id',@Maxpho_related_order_audit OUTPUT,@Rowcount3

UPDATE EICaseR_CASENUMBER9pho_related_order_audit_post
...
where audit_id in (select src_id from EICaseR_CASENUMBER9pho_related_order_audit_post)

CREATE TABLE EICaseR_CASENUMBER9pho_related_order_post (row_id INT IDENTITY,src_id BIGINT,dst_id BIGINT)

INSERT INTO EICaseR_CASENUMBER9pho_related_order_post (src_id)
SELECT distinct order_related_id FROM #stru_tempa2

DECLARE @Maxphys_order_id INT,@Rowcount4 INT,@facid4 INT;
SET @Rowcount4 = (SELECT count(1) from EICaseR_CASENUMBER9pho_related_order_post)
EXEC get_next_primary_key 'pho_related_order ' ,'order_related_id',@Maxphys_order_id OUTPUT,@Rowcount4

UPDATE EICaseR_CASENUMBER9pho_related_order_post
SET dst_id = @Maxphys_order_id + ([row_id] - 1)
WHERE dst_id IS NULL 
...
SELECT DISTINCT b.dst_id
	,a.[event_type]
	,ISNULL(EICaseR_CASENUMBER91.dst_id, a.order_related_id)
	,a.[phys_order_id]
	...
	,a.[deleted]
	,copy_fac.dst_id
	,ISNULL(EICaseR_CASENUMBER92.dst_id, a.order_relationship_id)
FROM [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].pho_related_order_audit a
JOIN [dbo].EICaseR_CASENUMBER9facility copy_fac ON copy_fac.src_id = a.fac_id
	OR copy_fac.src_id = R_SRCFACID9 --src fac_id
JOIN [dbo].EICaseR_CASENUMBER9pho_order_relationship EICaseR_CASENUMBER92 ON EICaseR_CASENUMBER92.src_id = a.order_relationship_id
LEFT JOIN [dbo].EICaseR_CASENUMBER9pho_related_order_post EICaseR_CASENUMBER91 ON EICaseR_CASENUMBER91.src_id = a.order_related_id
join [dbo].EICaseR_CASENUMBER9pho_related_order_audit_post b on b.src_id = a.audit_id
WHERE a.audit_id <> - 1
	AND a.fac_id IN (
		R_SRCFACID9 --src fac_id
		,- 1
		)
	AND a.audit_id = b.src_id 
...
if OBJECT_ID ('pcc_temp_storage.dbo.EICaseR_CASENUMBER9_bkp_pho_related_order_audit_updated1','U') is not null
drop table pcc_temp_storage.dbo.EICaseR_CASENUMBER9_bkp_pho_related_order_audit_updated1

select a.audit_id into pcc_temp_storage.dbo.EICaseR_CASENUMBER9_bkp_pho_related_order_audit_updated1
from pho_related_order_audit a
join EICaseR_CASENUMBER9pho_related_order_audit_post b on b.dst_id = a.audit_id
join EICaseR_CASENUMBER9pho_phys_order c on a.phys_order_id = c.src_id
where a.fac_id = R_DSTFACID9 --dst fac_id

if OBJECT_ID ('pcc_temp_storage.dbo.EICaseR_CASENUMBER9_bkp_pho_related_order_audit_updated2','U') is not null
drop table pcc_temp_storage.dbo.EICaseR_CASENUMBER9_bkp_pho_related_order_audit_updated2

select a.audit_id into pcc_temp_storage.dbo.EICaseR_CASENUMBER9_bkp_pho_related_order_audit_updated2
from pho_related_order_audit a
join EICaseR_CASENUMBER9pho_related_order_audit_post b on b.dst_id = a.audit_id
join EICaseR_CASENUMBER9pho_phys_order c on a.related_phys_order_id = c.src_id
where a.fac_id = R_DSTFACID9 --dst fac_id
...
from pho_related_order_audit a
join EICaseR_CASENUMBER9pho_related_order_audit_post b on b.dst_id = a.audit_id
join EICaseR_CASENUMBER9StruckOUT c on a.phys_order_id = c.src_id
where a.fac_id = R_DSTFACID9 --dst fac_id
...
from pho_related_order_audit a
join EICaseR_CASENUMBER9pho_related_order_audit_post b on b.dst_id = a.audit_id
join EICaseR_CASENUMBER9StruckOUT c on a.related_phys_order_id = c.src_id
where a.fac_id = R_DSTFACID9 --dst fac_id
...
SELECT src_id, dst_id FROM test_usei1214.dbo.EICaseR_CASENUMBER9as_std_assessment

CREATE TABLE #map_as_ard_planner (src_id INT PRIMARY KEY, dst_id INT NOT NULL)
INSERT INTO #map_as_ard_planner
SELECT src_id, dst_id FROM test_usei1214.dbo.EICaseR_CASENUMBER9as_ard_planner
...
if OBJECT_ID ('pcc_temp_storage.dbo.bkp_EICaseR_CASENUMBER9_cp_rev_intervention','U') is not null
	drop table pcc_temp_storage.dbo.bkp_EICaseR_CASENUMBER9_cp_rev_intervention
select * into pcc_temp_storage.dbo.bkp_EICaseR_CASENUMBER9_cp_rev_intervention from cp_rev_intervention

update dst
set category_id = null
from cp_rev_intervention dst
join EICaseR_CASENUMBER9cp_rev_intervention map 
...
if OBJECT_ID ('pcc_temp_storage.dbo._bkp_EICaseR_CASENUMBER9_diagnosis_notification','U') is not null
	drop table pcc_temp_storage.dbo._bkp_EICaseR_CASENUMBER9_diagnosis_notification
select * into pcc_temp_storage.dbo._bkp_EICaseR_CASENUMBER9_diagnosis_notification from diagnosis_notification

update diagnosis_notification
set assess_id = NULL
--select * 
from diagnosis_notification
where fac_id in (R_DSTFACID9) 
and assess_id not in 
(select assess_id from as_assessment where fac_id in (R_DSTFACID9))
...
if OBJECT_ID ('pcc_temp_storage.dbo.bkp_EICaseR_CASENUMBER9_as_std_trigger_update','U') is not null
drop table pcc_temp_storage.dbo.bkp_EICaseR_CASENUMBER9_as_std_trigger_update
select * into pcc_temp_storage.dbo.bkp_EICaseR_CASENUMBER9_as_std_trigger_update from as_std_trigger

update as_std_trigger
set triggered_item_id = null, deleted = 'Y',deleted_by = 'EICaseR_CASENUMBER9',deleted_date = getdate()
--select * 
from as_std_trigger 
where trigger_type IN ('I','N','G')
and std_assess_id in (select dst_id from EICaseR_CASENUMBER9as_std_assessment where corporate = 'N') 
...
if OBJECT_ID ('pcc_temp_storage.dbo._bkp_EICaseR_CASENUMBER9_as_response','U') is not null
	drop table pcc_temp_storage.dbo._bkp_EICaseR_CASENUMBER9_as_response

select a.* into pcc_temp_storage.dbo._bkp_EICaseR_CASENUMBER9_as_response 
from as_response a join EICaseR_CASENUMBER9as_assessment assess on a.assess_id=assess.dst_id

update a
set item_value = replace(item_value,'<sentTo>' + convert(varchar,b.src_id * -1) + '<','<sentTo>' + convert(varchar,b.dst_id * -1) + '<')
from  as_response a inner join EICaseR_CASENUMBER9emc_ext_facilities b
on left(a.item_value,patindex('%</sentTo>%',item_value)) = '<sentTo>' + convert(varchar,b.src_id * -1) + '<'
join EICaseR_CASENUMBER9as_assessment assess  
...
from  as_response a 
inner join EICaseR_CASENUMBER9unit b on b.src_id = a.item_value
join EICaseR_CASENUMBER9as_assessment assess  
on a.assess_id=assess.dst_id--
...
from  as_response a inner join EICaseR_CASENUMBER9unit b
...
if OBJECT_ID ('pcc_temp_storage.dbo._bkp_as_std_pick_list_item_EICaseR_CASENUMBER9_before_post','U') is not null
drop table pcc_temp_storage.dbo._bkp_as_std_pick_list_item_EICaseR_CASENUMBER9_before_post

if OBJECT_ID ('pcc_temp_storage.dbo._bkp_as_std_question_EICaseR_CASENUMBER9_before_post','U') is not null
drop table pcc_temp_storage.dbo._bkp_as_std_question_EICaseR_CASENUMBER9_before_post

if OBJECT_ID ('pcc_temp_storage.dbo._bkp_as_std_pick_list_item_EICaseR_CASENUMBER9_updated','U') is not null
drop table pcc_temp_storage.dbo._bkp_as_std_pick_list_item_EICaseR_CASENUMBER9_updated

if OBJECT_ID ('pcc_temp_storage.dbo._bkp_as_std_question_EICaseR_CASENUMBER9_updated','U') is not null
drop table pcc_temp_storage.dbo._bkp_as_std_question_EICaseR_CASENUMBER9_updated

--select * into pcc_temp_storage.dbo._bkp_as_std_pick_list_item_EICaseR_CASENUMBER9_before_post from as_std_pick_list_item 
--select * into pcc_temp_storage.dbo._bkp_as_std_question_EICaseR_CASENUMBER9_before_post from as_std_question 

select dst.* into pcc_temp_storage.dbo._bkp_as_std_pick_list_item_EICaseR_CASENUMBER9_updated
from as_std_pick_list_item dst
join EICaseR_CASENUMBER9as_std_pick_list map on dst.pick_list_id = map.dst_id
...
join EICaseR_CASENUMBER9qlib_pick_list_item qmap on dst.qlib_pick_list_item_id = qmap.dst_id
...
select dst.* into pcc_temp_storage.dbo._bkp_as_std_question_EICaseR_CASENUMBER9_updated
--select distinct dst.std_assess_id
from as_std_question dst
join EICaseR_CASENUMBER9as_std_assessment map on dst.std_assess_id = map.dst_id
...
join EICaseR_CASENUMBER9qlib_question qqmap on qqmap.dst_id = qqdst.std_question_id
...
(select dst_id from EICaseR_CASENUMBER9as_std_score where corporate = 'N')
...
WHILE (1 = 1) 
	...
	BEGIN 
	  ...
	  FROM EICaseR_CASENUMBER9as_std_score
	...
	from as_std_score dst
		join EICaseR_CASENUMBER9as_std_score map on dst.std_score_id = map.dst_id
...
if OBJECT_ID ('pcc_temp_storage.dbo._bkp_EICaseR_CASENUMBER9_inc_response','U') is not null
drop table pcc_temp_storage.dbo._bkp_EICaseR_CASENUMBER9_inc_response
select * into pcc_temp_storage.dbo._bkp_EICaseR_CASENUMBER9_inc_response from INC_RESPONSE

update dst
set dst.item_value = mapp.dst_id
--select dst.* 
from INC_RESPONSE dst 
join EICaseR_CASENUMBER9inc_incident map on dst.inc_id = map.dst_id
...
join EICaseR_CASENUMBER9inc_std_pick_list_item mapp on dst.item_value = mapp.src_id
...
join EICaseR_CASENUMBER9inc_incident map on dst.primary_triggered_by = map.src_id
join EICaseR_CASENUMBER9as_assess_schedule_details mapp on dst.detail_id= mapp.dst_id
...
--join EICaseR_CASENUMBER9as_std_assessment amap on amap.dst_id = dst.std_assess_id
...
--SELECT DISTINCT copy_fac.dst_id,ISNULL(EICaseR_CASENUMBER91.dst_id, std_assess_id)
--FROM [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].as_std_assessment_facility a -- src DB
--JOIN [dbo].EICaseR_CASENUMBER9facility copy_fac ON copy_fac.src_id = a.fac_id OR copy_fac.src_id = R_SRCFACID9
--JOIN [dbo].EICaseR_CASENUMBER9as_std_assessment EICaseR_CASENUMBER91 ON EICaseR_CASENUMBER91.src_id = a.std_assess_id
--JOIN [dbo].EICaseR_CASENUMBER9facility EICaseR_CASENUMBER92 ON EICaseR_CASENUMBER92.src_id = a.fac_id
--WHERE fac_id IN (R_SRCFACID9,- 1)
--       AND NOT EXISTS (SELECT 1 FROM [dbo].as_std_assessment_facility origt WHERE origt.std_assess_id = EICaseR_CASENUMBER91.dst_id AND origt.fac_id = EICaseR_CASENUMBER92.dst_id)
--       AND EICaseR_CASENUMBER91.dst_id not in (select std_assess_id from as_std_assessment_system_assessment_mapping)
...
--join EICaseR_CASENUMBER9as_assess_schedule map on dst.schedule_id = map.dst_id
...
--              SELECT dst_id
--              FROM EICaseR_CASENUMBER9as_assess_schedule
--              WHERE corporate = 'N'
--              )
--       AND i.fac_id = R_DSTFACID9 and i.event_id <> c.event_id 
...
--set deleted = 'Y', deleted_by = 'EICaseR_CASENUMBER9', deleted_date = getdate()
----select *
--from as_assessment
--where std_assess_id = 11 --MDS3
--and fac_id = R_DSTFACID9 --dst fac_id
--and assess_id in (select dst_id from EICaseR_CASENUMBER9as_assessment)--mapping table
...
--	INNER JOIN test_usei1214.dbo.EICaseR_CASENUMBER9pho_phys_order AS map ON src.phys_order_id = map.src_id
...
--into dbo.EIcaseR_CASENUMBER9map_identifier
----select *
...
--update  dbo.EIcaseR_CASENUMBER9map_identifier  
...
--into dbo.EIcaseR_CASENUMBER9pho_pharmacy_order
----select *
...
--update  dbo.EIcaseR_CASENUMBER9pho_pharmacy_order 
...
-----select  * 
--from  [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.pho_pharmacy_order src 
--left join dbo.EICaseR_CASENUMBER9pho_phys_order map
--on src.phys_order_id=map.src_id
--join EICaseR_CASENUMBER9emc_ext_facilities map2
--on map2.src_id=src.ext_pharmacy_id
--join EIcaseR_CASENUMBER9pho_pharmacy_order map3
...
-- NULL as dst_id
--into dbo.EIcaseR_CASENUMBER9pho_pharmacy_note_detail
----select *
...
--update  dbo.EIcaseR_CASENUMBER9pho_pharmacy_note_detail 
...
----select * 
--from  [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.pho_pharmacy_note_detail src 
--join  EIcaseR_CASENUMBER9pho_pharmacy_order map
--on  src.pharmacy_order_id=map.src_id
--join EIcaseR_CASENUMBER9pho_pharmacy_note_detail map2
...
----DROP TABLE EIcaseR_CASENUMBER9pho_phys_vendor
...
----select  *  
--from [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.pho_phys_vendor  src
--join EIcaseR_CASENUMBER9pho_phys_vendor map
--on src.phys_vendor_id=map.src_id
--join EIcaseR_CASENUMBER9pho_phys_order map2
...
-- NULL as dst_id
--into dbo.EIcaseR_CASENUMBER9pho_supply_dispense
----select  *  
--from  [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.pho_supply_dispense src --
--join EICaseR_CASENUMBER9pho_order_supply map
...
--update  dbo.EIcaseR_CASENUMBER9pho_supply_dispense 
...
----select  * 
--from [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.pho_supply_dispense src
--join EIcaseR_CASENUMBER9pho_supply_dispense map
--on map.src_id=src.supply_dispense_id 
--join EIcaseR_CASENUMBER9pho_pharmacy_order map3
--on map3.src_id=src.pharmacy_order_id 
--join EIcaseR_CASENUMBER9pho_order_supply map4
...
--where fac_id = R_SRCFACID9 
--and map_type_id = 2 
--and internal_id in (select src_id from EICaseR_CASENUMBER9clients)
...
--	,internal_id
--FROM [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.map_identifier src
--JOIN EIcaseR_CASENUMBER9map_identifier map ON src.map_identifier_id = map.src_id
--WHERE src.fac_id =R_SRCFACID9
...
--where map_identifier_id in (select src_id from EIcaseR_CASENUMBER9map_identifier)) 
...
--(select src_id from EIcaseR_CASENUMBER9pho_pharmacy_order)
