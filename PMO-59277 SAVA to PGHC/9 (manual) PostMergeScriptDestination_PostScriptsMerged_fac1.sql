...
join EICase59277183as_assessment map1 on map1.src_id = src.assess_id ------mapping table
join EICase59277183as_assessment map2 on map2.src_id = src.first_assess_id  ------mapping table
...
-- Post check for facility_history_extract and facility_history_extract_audit  

/*

Hi Team,

Please check your projects - both DC- New and Existing to see if you encounter any rows copied by our process in the tables facility_history_extract and facility_history_extract_audit  for the facility(s) being copied. 
If you do see any results, please let me know and run a post script to delete the rows for your facility.

This is due to the fact when PCC user is turning on Analytics and if there are rows in these tables for the facility, they will get an error.

Delete from dbo.facility_history_extract where fac_id in (fac_id); --dest fac_id 
Delete from dbo.facility_history_extract_audit where fac_id in (fac_id); --dest fac_id

Please let me know if you have any questions.

*/

Delete from facility_history_extract where fac_id in (173)
Delete from facility_history_extract_audit where fac_id in (173)
...
JOIN [dbo].EICase59277183common_code m ON s.common_code_item_id = m.src_id
JOIN [dbo].common_code d ON m.dst_id = d.item_id
WHERE m.dst_id NOT IN (SELECT common_code_item_id FROM [dbo].[common_code_standard_contact_type_mapping]) AND d.fac_id = 173 
...
--inner join dbo.EICase59277183census_item map--
...
--where dst.fac_id in (173)--dst fac_id
--and alp.payer_id not in (select payer_id from ar_payers where fac_id in (173))--dst fac_id
--and map_dstpayerid is not null 
...
--inner join [dbo].[EICase59277183room_date_range] map--
...
--WHERE dst.room_id in  (select  room_id from room where fac_id = 173)
...
join test_usei1214.dbo.EICase59277183sec_user umap on dst.userid = umap.dst_id
join [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.sec_user src on umap.src_id = src.userid
join test_usei1214.dbo.EICase59277183contact cmap on src.staff_id = cmap.src_id
...
where user_id not in (select dst_id from EICase59277183sec_user) 
and created_by like '%EICase59277183%'
...
join EICase59277183sec_user map on dst.user_id = map.src_id
...
IF OBJECT_ID('pcc_temp_storage.dbo._bkp_sec_user_snapshot_Case59277183', 'U') IS NOT NULL 
	DROP TABLE pcc_temp_storage.dbo._bkp_sec_user_snapshot_Case59277183
SELECT * into pcc_temp_storage.dbo._bkp_sec_user_snapshot_Case59277183 from sec_user

update sec_user
set email = '', alt_email = ''
where ((email is not null and email <> '') or (alt_email is not null and alt_email <> ''))
and userid in (select dst_id from EICase59277183sec_user where corporate = 'N')
...
left join [dbo].EICase59277183mpi_history m
on s.mpi_history_id = m.src_id
left join [dbo].mpi_history d
on d.mpi_history_id = m.dst_id and d.fac_id = 173
...
SELECT 
	DISTINCT b.dst_id,ISNULL(EICase592771831.dst_id, mpi_id),copy_fac.dst_id,ISNULL(EICase5927718310.dst_id, user_id),[created_by],[created_date],[revision_by],[revision_date]
	,[title],[first_name],[last_name],[middle_name],[maiden_name],[date_of_birth],[place_of_birth],ISNULL(EICase592771834.dst_id, marital_status_id),[sex],[address1],[address2]
	,[address3],[city]	,[county_id],[prov_state],[postal_zip_code],ISNULL(EICase592771839.dst_id, country_id),[deceased_date],ISNULL(EICase592771835.dst_id, religion_id)
	,ISNULL(EICase592771836.dst_id, race_id),ISNULL(EICase592771832.dst_id, primary_lang_id),ISNULL(EICase592771833.dst_id, secondary_lang_id)
	,ISNULL(EICase592771838.dst_id, citizenship_id),[allergy],[height],[ibw_range]	,[veteran_number],[public_trustee_number],ISNULL(EICase592771837.dst_id, education_id),[occupations]
...
JOIN test_usei1214.[dbo].EICase59277183facility copy_fac ON copy_fac.src_id = a.fac_id OR copy_fac.src_id = 183
JOIN test_usei1214.[dbo].EICase59277183mpi EICase592771831 ON EICase592771831.src_id = a.mpi_id
LEFT JOIN test_usei1214.[dbo].EICase59277183common_code EICase592771832 ON EICase592771832.src_id = a.primary_lang_id
LEFT JOIN test_usei1214.[dbo].EICase59277183common_code EICase592771833 ON EICase592771833.src_id = a.secondary_lang_id
LEFT JOIN test_usei1214.[dbo].EICase59277183common_code EICase592771834 ON EICase592771834.src_id = a.marital_status_id
LEFT JOIN test_usei1214.[dbo].EICase59277183common_code EICase592771835 ON EICase592771835.src_id = a.religion_id
LEFT JOIN test_usei1214.[dbo].EICase59277183common_code EICase592771836 ON EICase592771836.src_id = a.race_id
LEFT JOIN test_usei1214.[dbo].EICase59277183common_code EICase592771837 ON EICase592771837.src_id = a.education_id
LEFT JOIN test_usei1214.[dbo].EICase59277183common_code EICase592771838 ON EICase592771838.src_id = a.citizenship_id
LEFT JOIN test_usei1214.[dbo].EICase59277183common_code EICase592771839 ON EICase592771839.src_id = a.country_id
LEFT JOIN test_usei1214.[dbo].EICase59277183sec_user EICase5927718310 ON EICase5927718310.src_id = a.user_id ,test_usei1214.[dbo].EICase59277183mpi_history b
WHERE a.mpi_history_id <> - 1
	AND a.fac_id IN (183,- 1)
...
if OBJECT_ID ('pcc_temp_storage.dbo._bkp_diagnosis_PGHC_post_EICase59277183diagnosis_codes_post','U') is not null
	drop table pcc_temp_storage.dbo._bkp_diagnosis_PGHC_post_EICase59277183diagnosis_codes_post

if OBJECT_ID ('dbo.EICase59277183diagnosis_codes_post','U') is not null
	drop table dbo.EICase59277183diagnosis_codes_post

--1. Generate post mapping table, populating affected src diagnosis_id
select distinct map.* 
into EICase59277183diagnosis_codes_post --update Case #
--select distinct map.*
--select src.*
from EICase59277183diagnosis_codes map
...
update EICase59277183diagnosis_codes_post set corporate = 'N'
...
join EICase59277183diagnosis_codes_post map2 on map2.src_id = src.diagnosis_id
where src.diagnosis_id in (select distinct src_id from EICase59277183diagnosis_codes_post) 
...
from diagnosis dst
join EICase59277183diagnosis mapd on dst.client_diagnosis_id = mapd.dst_id
join EICase59277183diagnosis_codes map on map.dst_id = dst.diagnosis_id
join EICase59277183diagnosis_codes_post map2 on map2.src_id = map.src_id
...
(
select src_id from EICase59277183diagnosis_codes 
	where src_id not in (select src_id from EICase59277183diagnosis_codes_post where corporate = 'Y')
	and dst_id NOT IN (SELECT diagnosis_id FROM dbo.diagnosis_codes_clinical_category)
union
select src_id from EICase59277183diagnosis_codes_post where corporate = 'Y'
	and dst_id NOT IN (SELECT diagnosis_id FROM dbo.diagnosis_codes_clinical_category)
	)
...
and file_id not in (select dst_id from EICase59277183file_metadata) 
and created_by = 'EICase59277183'
...
WHERE fac_id = 173 
    ...
    AND file_metadata_id IN (SELECT dst_id FROM EICase59277183file_metadata) 
...
WHERE audit_id IN (SELECT src_id FROM EICase59277183pho_phys_order_audit)--are copied --mapping table
AND phys_order_id NOT IN (SELECT phys_order_id FROM [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.pho_phys_order with (nolock) WHERE fac_id = 183) --src fac_id

select * into #stru_tempb from pho_phys_order_audit with (nolock)
where fac_id = 173 --(253548 row(s) affected) --dst fac_id
and client_id in (select dst_id from EICase59277183Clients with (nolock))----mapping table

SELECT dst_id into #stru_tempdstid FROM EICase59277183pho_phys_order_audit with (nolock)--copied over by E/I --mapping table
...
CREATE TABLE EICase59277183StruckOUT (row_id INT IDENTITY,src_id BIGINT,dst_id BIGINT)

INSERT INTO EICase59277183StruckOUT (src_id)
SELECT distinct phys_order_id FROM #stru_temps-- Jimmy  Added  Distinct

DECLARE @Maxphys_order_id INT,@Rowcount0 INT,@facid0 INT;
SET @Rowcount0 = (SELECT count(1) from EICase59277183StruckOUT)
EXEC get_next_primary_key 'pho_phys_order ' ,'phys_order_id',@Maxphys_order_id OUTPUT,@Rowcount0

UPDATE EICase59277183StruckOUT
...
JOIN EICase59277183StruckOUT b
ON a.phys_order_id = b.src_id
where 
a.client_id in (select dst_id from EICase59277183Clients )--
and a.fac_id = 173 --dst fac_id
...
and b.fac_id = 183 --src fac_id

CREATE TABLE EICase59277183pho_admin_order_audit_post (row_id INT IDENTITY,src_id BIGINT,dst_id BIGINT)

INSERT INTO EICase59277183pho_admin_order_audit_post (src_id)
SELECT 
distinct audit_id FROM #stru_temp

DECLARE @Maxpho_admin_order_audit INT,@Rowcount1 INT,@facid1 INT;
SET @Rowcount1 = (SELECT count(1) from EICase59277183pho_admin_order_audit_post)
EXEC get_next_primary_key 'pho_admin_order_audit ' ,'audit_id',@Maxpho_admin_order_audit OUTPUT,@Rowcount1

UPDATE EICase59277183pho_admin_order_audit_post
...
CREATE TABLE EICase59277183pho_admin_order_post (row_id INT IDENTITY,src_id BIGINT,dst_id BIGINT)

INSERT INTO EICase59277183pho_admin_order_post (src_id)
SELECT 
distinct admin_order_id FROM #stru_temp2

DECLARE @Maxpho_admin_order INT,@Rowcount2 INT,@facid2 INT;
SET @Rowcount2 = (SELECT count(1) from EICase59277183pho_admin_order_post)
EXEC get_next_primary_key 'pho_admin_order ' ,'admin_order_id',@Maxpho_admin_order OUTPUT,@Rowcount2

UPDATE EICase59277183pho_admin_order_post
...
SELECT DISTINCT b.dst_id
	,a.[event_type]
	,ISNULL(EICase592771831.dst_id, a.admin_order_id)
	,ISNULL(EICase592771833.dst_id, a.phys_order_id)
	,a.[effective_date]
	,isnull(a.ineffective_date, getdate())
	,ISNULL(EICase592771832.dst_id, a.physician_id)
	,a.[created_by]
	,a.[created_date]
	,a.[revision_by]
	,a.[revision_date]
	...
	,a.[deleted_date]
	,a.[reason]
	,ISNULL(EICase592771834.dst_id, a.noted_by)
	,a.[strikeout_by]
	,a.[strikeout_date]
...
LEFT JOIN [dbo].EICase59277183pho_admin_order_post EICase592771831 ON EICase592771831.src_id = a.admin_order_id
LEFT JOIN [dbo].EICase59277183contact EICase592771832 ON EICase592771832.src_id = a.physician_id
JOIN [dbo].EICase59277183StruckOUT EICase592771833 ON EICase592771833.src_id = a.phys_order_id --S/O post mapping table
LEFT JOIN [dbo].EICase59277183sec_user EICase592771834 ON EICase592771834.src_id = a.noted_by --nullable
	...
	,[dbo].EICase59277183pho_admin_order_audit_post b
WHERE 
	a.audit_id <> - 1
	AND a.audit_id = b.src_id
	AND pa.client_id in (select src_id from EICase59277183clients) 
...
and b.fac_id = 183 --src fac_id
and a.fac_id = 183 --src fac_id
and b.client_id in (select src_id from EICase59277183clients)
union
SELECT distinct a.audit_id
--INTO #stru_tempa3
--select distinct a.*
...
CREATE TABLE EICase59277183pho_related_order_audit_post (row_id INT IDENTITY,src_id BIGINT,dst_id BIGINT)

INSERT INTO EICase59277183pho_related_order_audit_post (src_id)
SELECT 
distinct audit_id FROM #stru_tempa


DECLARE @Maxpho_related_order_audit INT,@Rowcount3 INT,@facid3 INT;
SET @Rowcount3 = (SELECT count(1) from EICase59277183pho_related_order_audit_post)
EXEC get_next_primary_key 'pho_related_order_audit ' ,'audit_id',@Maxpho_related_order_audit OUTPUT,@Rowcount3

UPDATE EICase59277183pho_related_order_audit_post
...
where audit_id in (select src_id from EICase59277183pho_related_order_audit_post)

CREATE TABLE EICase59277183pho_related_order_post (row_id INT IDENTITY,src_id BIGINT,dst_id BIGINT)

INSERT INTO EICase59277183pho_related_order_post (src_id)
SELECT distinct order_related_id FROM #stru_tempa2

DECLARE @Maxphys_order_id INT,@Rowcount4 INT,@facid4 INT;
SET @Rowcount4 = (SELECT count(1) from EICase59277183pho_related_order_post)
EXEC get_next_primary_key 'pho_related_order ' ,'order_related_id',@Maxphys_order_id OUTPUT,@Rowcount4

UPDATE EICase59277183pho_related_order_post
SET dst_id = @Maxphys_order_id + ([row_id] - 1)
WHERE dst_id IS NULL 
...
SELECT DISTINCT b.dst_id
	,a.[event_type]
	,ISNULL(EICase592771831.dst_id, a.order_related_id)
	,a.[phys_order_id]
	...
	,a.[deleted]
	,copy_fac.dst_id
	,ISNULL(EICase592771832.dst_id, a.order_relationship_id)
FROM [vmuspassvtscon3.pccprod.local].test_usei3sava1.[dbo].pho_related_order_audit a
JOIN [dbo].EICase59277183facility copy_fac ON copy_fac.src_id = a.fac_id
	OR copy_fac.src_id = 183 --src fac_id
JOIN [dbo].EICase59277183pho_order_relationship EICase592771832 ON EICase592771832.src_id = a.order_relationship_id
LEFT JOIN [dbo].EICase59277183pho_related_order_post EICase592771831 ON EICase592771831.src_id = a.order_related_id
join [dbo].EICase59277183pho_related_order_audit_post b on b.src_id = a.audit_id
WHERE a.audit_id <> - 1
	AND a.fac_id IN (
		183 --src fac_id
		,- 1
...
if OBJECT_ID ('pcc_temp_storage.dbo.EICase59277183_bkp_pho_related_order_audit_updated1','U') is not null
drop table pcc_temp_storage.dbo.EICase59277183_bkp_pho_related_order_audit_updated1

select a.audit_id into pcc_temp_storage.dbo.EICase59277183_bkp_pho_related_order_audit_updated1
from pho_related_order_audit a
join EICase59277183pho_related_order_audit_post b on b.dst_id = a.audit_id
join EICase59277183pho_phys_order c on a.phys_order_id = c.src_id
where a.fac_id = 173 --dst fac_id

if OBJECT_ID ('pcc_temp_storage.dbo.EICase59277183_bkp_pho_related_order_audit_updated2','U') is not null
drop table pcc_temp_storage.dbo.EICase59277183_bkp_pho_related_order_audit_updated2

select a.audit_id into pcc_temp_storage.dbo.EICase59277183_bkp_pho_related_order_audit_updated2
from pho_related_order_audit a
join EICase59277183pho_related_order_audit_post b on b.dst_id = a.audit_id
join EICase59277183pho_phys_order c on a.related_phys_order_id = c.src_id
where a.fac_id = 173 --dst fac_id
...
from pho_related_order_audit a
join EICase59277183pho_related_order_audit_post b on b.dst_id = a.audit_id
join EICase59277183StruckOUT c on a.phys_order_id = c.src_id
where a.fac_id = 173 --dst fac_id
...
from pho_related_order_audit a
join EICase59277183pho_related_order_audit_post b on b.dst_id = a.audit_id
join EICase59277183StruckOUT c on a.related_phys_order_id = c.src_id
where a.fac_id = 173 --dst fac_id
...
SELECT src_id, dst_id FROM test_usei1214.dbo.EICase59277183as_std_assessment

CREATE TABLE #map_as_ard_planner (src_id INT PRIMARY KEY, dst_id INT NOT NULL)
INSERT INTO #map_as_ard_planner
SELECT src_id, dst_id FROM test_usei1214.dbo.EICase59277183as_ard_planner
...
set deleted = 'Y'
--select * 
from test_usei1214.dbo.common_code d
where deleted = 'N' and DELETED_BY is not null 
...
if OBJECT_ID ('pcc_temp_storage.dbo.bkp_EICase59277183_cp_rev_intervention','U') is not null
	drop table pcc_temp_storage.dbo.bkp_EICase59277183_cp_rev_intervention
select * into pcc_temp_storage.dbo.bkp_EICase59277183_cp_rev_intervention from cp_rev_intervention

update dst
set category_id = null
from cp_rev_intervention dst
join EICase59277183cp_rev_intervention map 
...
if OBJECT_ID ('pcc_temp_storage.dbo._bkp_EICase59277183_diagnosis_notification','U') is not null
	drop table pcc_temp_storage.dbo._bkp_EICase59277183_diagnosis_notification
select * into pcc_temp_storage.dbo._bkp_EICase59277183_diagnosis_notification from diagnosis_notification
...
(select assess_id from as_assessment where fac_id in (173))
...
if OBJECT_ID ('pcc_temp_storage.dbo.bkp_EICase59277183_as_std_trigger_update','U') is not null
drop table pcc_temp_storage.dbo.bkp_EICase59277183_as_std_trigger_update
select * into pcc_temp_storage.dbo.bkp_EICase59277183_as_std_trigger_update from as_std_trigger

update as_std_trigger
set triggered_item_id = null, deleted = 'Y',deleted_by = 'EICase59277183',deleted_date = getdate()
--select * 
from as_std_trigger 
where trigger_type IN ('I','N','G')
and std_assess_id in (select dst_id from EICase59277183as_std_assessment where corporate = 'N') 
...
if OBJECT_ID ('pcc_temp_storage.dbo._bkp_EICase59277183_as_response','U') is not null
	drop table pcc_temp_storage.dbo._bkp_EICase59277183_as_response

select a.* into pcc_temp_storage.dbo._bkp_EICase59277183_as_response 
from as_response a join EICase59277183as_assessment assess on a.assess_id=assess.dst_id

update a
set item_value = replace(item_value,'<sentTo>' + convert(varchar,b.src_id * -1) + '<','<sentTo>' + convert(varchar,b.dst_id * -1) + '<')
from  as_response a inner join EICase59277183emc_ext_facilities b
on left(a.item_value,patindex('%</sentTo>%',item_value)) = '<sentTo>' + convert(varchar,b.src_id * -1) + '<'
join EICase59277183as_assessment assess  
...
from  as_response a 
inner join EICase59277183unit b on b.src_id = a.item_value
join EICase59277183as_assessment assess  
on a.assess_id=assess.dst_id--
...
from  as_response a inner join EICase59277183unit b
...
if OBJECT_ID ('pcc_temp_storage.dbo._bkp_as_std_pick_list_item_EICase59277183_before_post','U') is not null
drop table pcc_temp_storage.dbo._bkp_as_std_pick_list_item_EICase59277183_before_post

if OBJECT_ID ('pcc_temp_storage.dbo._bkp_as_std_question_EICase59277183_before_post','U') is not null
drop table pcc_temp_storage.dbo._bkp_as_std_question_EICase59277183_before_post

if OBJECT_ID ('pcc_temp_storage.dbo._bkp_as_std_pick_list_item_EICase59277183_updated','U') is not null
drop table pcc_temp_storage.dbo._bkp_as_std_pick_list_item_EICase59277183_updated

if OBJECT_ID ('pcc_temp_storage.dbo._bkp_as_std_question_EICase59277183_updated','U') is not null
drop table pcc_temp_storage.dbo._bkp_as_std_question_EICase59277183_updated

--select * into pcc_temp_storage.dbo._bkp_as_std_pick_list_item_EICase59277183_before_post from as_std_pick_list_item 
--select * into pcc_temp_storage.dbo._bkp_as_std_question_EICase59277183_before_post from as_std_question 

select dst.* into pcc_temp_storage.dbo._bkp_as_std_pick_list_item_EICase59277183_updated
from as_std_pick_list_item dst
join EICase59277183as_std_pick_list map on dst.pick_list_id = map.dst_id
...
join EICase59277183qlib_pick_list_item qmap on dst.qlib_pick_list_item_id = qmap.dst_id
...
select dst.* into pcc_temp_storage.dbo._bkp_as_std_question_EICase59277183_updated
--select distinct dst.std_assess_id
from as_std_question dst
join EICase59277183as_std_assessment map on dst.std_assess_id = map.dst_id
...
join EICase59277183qlib_question qqmap on qqmap.dst_id = qqdst.std_question_id
...
if OBJECT_ID ('pcc_temp_storage.dbo._bkp_EICase59277183_inc_response','U') is not null
drop table pcc_temp_storage.dbo._bkp_EICase59277183_inc_response
select * into pcc_temp_storage.dbo._bkp_EICase59277183_inc_response from INC_RESPONSE

update dst
set dst.item_value = mapp.dst_id
--select dst.* 
from INC_RESPONSE dst 
join EICase59277183inc_incident map on dst.inc_id = map.dst_id
...
join EICase59277183inc_std_pick_list_item mapp on dst.item_value = mapp.src_id
...
join EICase59277183inc_incident map on dst.primary_triggered_by = map.src_id
join EICase59277183as_assess_schedule_details mapp on dst.detail_id= mapp.dst_id
...
--join EICase59277183as_std_assessment amap on amap.dst_id = dst.std_assess_id
...
--and fac_id = 173 --dst fac_id
--and assess_id in (select dst_id from EICase59277183as_assessment)--mapping table
...
--	INNER JOIN test_usei1214.dbo.EICase59277183pho_phys_order AS map ON src.phys_order_id = map.src_id
...
--where fac_id = 183 
--and map_type_id = 2 
--and internal_id in (select src_id from EICase59277183clients)
...
--	,internal_id
--FROM [vmuspassvtscon3.pccprod.local].test_usei3sava1.dbo.map_identifier src
--JOIN EICase59277183map_identifier map ON src.map_identifier_id = map.src_id
--WHERE src.fac_id =183
...
--where map_identifier_id in (select src_id from EICase59277183map_identifier)) 
...
--(select src_id from EICase59277183pho_pharmacy_order)
