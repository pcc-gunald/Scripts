...
AS
BEGIN
--7/February/2022 - 9:00 PM to 8/February/2022 - 4:00 AM
	DECLARE @pmo_number VARCHAR(MAX) = '59112'


	DECLARE @src_org_code VARCHAR(MAX) = 'CERTUS'
	DECLARE @dst_org_code VARCHAR(MAX) = 'BSD'

	DECLARE @src_prod VARCHAR(MAX) = '[pccsql-use2-prod-w30-cli0021.cbafa2b80e84.database.windows.net].[us_certus_multi]'
	DECLARE @dst_prod VARCHAR(MAX) = '[pccsql-use2-prod-w22-cli0005.4c4638f8e26f.database.windows.net].[us_bsd_multi]'

	DECLARE @src_test_org_code VARCHAR(MAX) = 'usei1066'
	DECLARE @dst_test_org_code VARCHAR(MAX) = 'usei1173'

	DECLARE @golive_date VARCHAR(MAX) = 'February 7, 2022'
	DECLARE @golive_day VARCHAR(MAX) = 'Monday'
	DECLARE @golive_starttime VARCHAR(MAX) = 'Monday, February 7th, 9PM'
	DECLare @golive_endtime varchar(max) = '9PM'
	DECLARE @golive_time VARCHAR(MAX) = '9PM to 4AM'
	DECLARE @offline_minutes VARCHAR(MAX) = '5'
	DECLARE @facility_names VARCHAR(MAX) = 'Suburban Healthcare and Rehabilitation'

	DECLARE @all_SRC_facID VARCHAR(MAX) = '12'
	DECLARE @all_DST_facID VARCHAR(MAX) = '18'
	DECLARE @case_number VARCHAR(MAX) = '' -- netsuite photos & online docs case

	DECLARE @tst VARCHAR(MAX) = '1138'
	...
	DECLARE @implementers varchar(200) = 'Stephani & Alexandra'
	DECLARE @deadline varchar(200) = 'Monday, February 7th'
