...
BEGIN
	...
	SET @vPMOGroupID = 1788

	/*------Job Fail Email Recipient------*/
	SET @vToEmailAddress = N'dinesh.gunalapan@pointclickcare.com;'

	/*------External Email Notification Recipient, for start of the run, email address only, semicolon separated------*/
	SET @EmailrecipientsExt = 'linda.k@pointclickcare.com;twest@cornerstonemgmt.com; anygren@cornerstonemgmt.com; mrogers@cornerstonemgmt.com; morr@cornerstonemgmt.com;Tammi.Irvine@pointclickcare.com;melodee.mercado@pointclickcare.com;Katie.Wedell@pointclickcare.com;rina.p@pointclickcare.com;nigel.liang@pointclickcare.com;TSFacAcqConfig@pointclickcare.com;'
	...
	SET @action_codes_mapping_table			= '[vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E009215_ActionCodes$]'
	SET @status_codes_mapping_table			= '[vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E009215_StatusCodes$]'
	SET @upload_categories_mapping_table	= '[vmuspassvtsjob1.pccprod.local].FacAcqMapping.[dbo].[E009215_UploadCategories$]'
	SET @room_rate_type_mapping_table		='[vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[E009215_RoomRateType$]'
	/*------Medicare A id from payer mapping, this is used in room rate type post update------*/
	SET @Medicare_A_in_destination			= ''
	...
	DECLARE @if_as_ard_adl_keys_post_insert varchar(1)

 

	/*------For AUTO-PRE, and Post Merge on stagingdb, if doing post insert, set to Y -----*/
    SET @if_as_ard_adl_keys_post_insert   = 'Y'
	...
	BEGIN TRY
		...
		BEGIN
			...
			BEGIN --Start CurCase
				...
				IF @if_as_ard_adl_keys_post_insert = 'Y'
				...
				IF @if_as_ard_adl_keys_post_insert = 'Y'
				BEGIN 
				SET @SqlCommand = @SqlCommand + 'PRINT ''--Post insert for as_ard_adl_keys'''+CHAR(13)+CHAR(10)
				...
				SET @SqlCommand = @SqlCommand + 'INNER JOIN EICase' + @vCaseNo + 'as_std_assessment b ON b.src_id = a.std_assess_id'+CHAR(13)+CHAR(10)
				SET @SqlCommand = @SqlCommand + 'INNER JOIN EICase' + @vCaseNo + 'as_ard_planner c ON c.src_id = a.ard_planner_id'+CHAR(13)+CHAR(10)+CHAR(13)+CHAR(10) 
				END
...
/*
Modules that will NOT be imported or NOT being utilized
** Note: Any documentation on the following list that will be required in the future for regulatory or other reasons needs to be printed to hard copy.
•	POC Historical Documentation - Look Back reports will not be available
•	POC Libraries and Resident Tasks 
•	AR Transactional History
•	Collections/Billing
--•	Trust – NOT utilized
•	GLAP
•	IRM/CRM
--•	Admin Notes – NOT utlized
•	MDS – NOT utilized
•	Lab/Radiology Results – NOT utilized
--•	Custom UDAs (If UDAs are not authorized, the assessments tab in the resident’s chart will not contain any historical assessments as well as the clinical dashboard.) – NOT authorized
--•	Alerts – NOT authorized
--•	Risk Management – NOT authorized

*/

---E7c: Custom UDAs,  E16: Alert, E17: Risk Management, E18: Trust,  E23: Notes
--E7c: Custom UDAs, E16: Alert, E17: Risk Management, E18: Trust, E23: Notes
