...
BEGIN
	...
	DECLARE @pmo_number VARCHAR(MAX) = '59618'

	DECLARE @src_org_code VARCHAR(MAX) = 'HVHC'
	DECLARE @dst_org_code VARCHAR(MAX) = 'VIEW'

	DECLARE @src_prod VARCHAR(MAX) = '[pccsql-use2-prod-w26-cli0003.d9c23db323d7.database.windows.net].[us_hvhc_multi]'
	DECLARE @dst_prod VARCHAR(MAX) = '[pccsql-use2-prod-w27-cli0027.851c37dfbe45.database.windows.net].[us_view]'

	DECLARE @src_test_org_code VARCHAR(MAX) = 'USEI963'
	DECLARE @dst_test_org_code VARCHAR(MAX) = 'USEI45'

	DECLARE @golive_date VARCHAR(MAX) = 'April, 13, 2022'
	DECLARE @golive_day VARCHAR(MAX) = 'Wednesday'
	DECLARE @golive_starttime VARCHAR(MAX) = 'Wednesday, April 13th, 2:00 PM'
	DECLare @golive_endtime varchar(max) = '2:00 AM'
	DECLARE @golive_time VARCHAR(MAX) = '2:00 PM to 2:00 AM'
	DECLARE @offline_minutes VARCHAR(MAX) = '15'
	DECLARE @facility_names VARCHAR(MAX) = ' South County Eden Operation LLC, '

	DECLARE @all_SRC_facID VARCHAR(MAX) = '4,'
	DECLARE @all_DST_facID VARCHAR(MAX) = '2,'
	DECLARE @case_number VARCHAR(MAX) = '2003528' -- netsuite photos & online docs case

	DECLARE @tst VARCHAR(MAX) = '1504'
	...
	DECLARE @deadline varchar(200) = 'Wednesday, April 13th'
