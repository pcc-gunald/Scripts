...
BEGIN
	...
	DECLARE @pmo_number VARCHAR(MAX) = '59065'


	DECLARE @src_org_code VARCHAR(MAX) = 'CTYD'
	DECLARE @dst_org_code VARCHAR(MAX) = 'MAGIC'

	DECLARE @src_prod VARCHAR(MAX) = '[pccsql-use2-prod-w24-cli0014.ce22455c967a.database.windows.net].[us_ctyd]'
	DECLARE @dst_prod VARCHAR(MAX) = '[pccsql-use2-prod-w22-cli0006.4c4638f8e26f.database.windows.net].[us_magic_multi]'

	DECLARE @src_test_org_code VARCHAR(MAX) = 'usei1172'
	DECLARE @dst_test_org_code VARCHAR(MAX) = 'usei432'

	DECLARE @golive_date VARCHAR(MAX) = 'February 1, 2022'
	DECLARE @golive_day VARCHAR(MAX) = 'Tuesday'
	DECLARE @golive_starttime VARCHAR(MAX) = 'Tuesday, February 1st, 9PM'
	DECLare @golive_endtime varchar(max) = '9PM'
	DECLARE @golive_time VARCHAR(MAX) = '9PM to 4AM'
	DECLARE @offline_minutes VARCHAR(MAX) = '15'
	DECLARE @facility_names VARCHAR(MAX) = 'Courtyard Healthcare Center'

	DECLARE @all_SRC_facID VARCHAR(MAX) = '1'
	DECLARE @all_DST_facID VARCHAR(MAX) = '39'
	DECLARE @case_number VARCHAR(MAX) = '' -- netsuite photos & online docs case

	DECLARE @tst VARCHAR(MAX) = '1119'
	...
	DECLARE @implementers varchar(200) = 'Stephani & Matt'
	DECLARE @deadline varchar(200) = 'Tuesday, February 1st, 9PM'
