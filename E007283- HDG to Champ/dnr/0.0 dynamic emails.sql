...
BEGIN
	...
	DECLARE @pmo_number VARCHAR(MAX) = '007283'

	DECLARE @src_org_code VARCHAR(MAX) = 'HDG'
	DECLARE @dst_org_code VARCHAR(MAX) = 'CHAMP'

	DECLARE @src_prod VARCHAR(MAX) = '[pccsql-use2-prod-w25-cli0023.2d62ac5c7643.database.windows.net].[us_hdg_multi]'
	DECLARE @dst_prod VARCHAR(MAX) = '[pccsql-use2-prod-w28-cli0007.874bf44c721a.database.windows.net].[us_champ_multi]'

	DECLARE @src_test_org_code VARCHAR(MAX) = 'usei547'
	DECLARE @dst_test_org_code VARCHAR(MAX) = 'usei1062'

	DECLARE @golive_date VARCHAR(MAX) = 'December 03, 2021'
	DECLARE @golive_day VARCHAR(MAX) = 'Friday'
	DECLARE @golive_starttime VARCHAR(MAX) = 'Friday, December 3rd, 1PM'
	DECLare @golive_endtime varchar(max) = '1PM'
	DECLARE @golive_time VARCHAR(MAX) = '1PM to 11PM'
	...
	DECLARE @deadline varchar(200) = 'Monday, December 3rd'
