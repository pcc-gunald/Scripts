...
BEGIN
	...
	DECLARE @src_prod VARCHAR(MAX) = '[pccsql-use2-prod-w26-cli0018.d9c23db323d7.database.windows.net].[us_gbsl_multi]'
	DECLARE @dst_prod VARCHAR(MAX) = '[pccsql-use2-prod-w25-cli0021.2d62ac5c7643.database.windows.net].[us_gdn_multi]'

	DECLARE @src_test_org_code VARCHAR(MAX) = 'usei740'
	DECLARE @dst_test_org_code VARCHAR(MAX) = 'usei1104'

	DECLARE @golive_date VARCHAR(MAX) = 'January 10, 2021'
	DECLARE @golive_day VARCHAR(MAX) = 'Monday'
	DECLARE @golive_starttime VARCHAR(MAX) = 'Monday, January 10th, 10AM'
	DECLare @golive_endtime varchar(max) = '10AM'
	DECLARE @golive_time VARCHAR(MAX) = '10AM to 4AM'
	DECLARE @offline_minutes VARCHAR(MAX) = '5'
	DECLARE @facility_names VARCHAR(MAX) = 'Schoenbrunn Health Care, Glendora Care Center ,Pearlview Rehab &amp; Wellness Center'
	...
	DECLARE @implementers varchar(200) = 'Ashlee & Joyce'
	DECLARE @deadline varchar(200) = 'Monday, January 10th'
