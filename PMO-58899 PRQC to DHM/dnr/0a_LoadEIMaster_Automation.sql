...
(PMONumber,Completed)
SELECT 
'58899' as PMONumber, --PMO Number
0 as Completed

SELECT SCOPE_IDENTITY()

select * from LoadEIMaster_PMO_Groups where PMONumber like '%58899%'--1720

drop table if exists #temp_fac
create table #temp_fac(caseNo varchar(50),src_id int, dest_id int, id int identity (1,1))

insert into #temp_fac values('58899',1,192)
insert into #temp_fac values('58899',2,193)
insert into #temp_fac values('58899',5,194)
...
SELECT '1742' as [PMO_Group_Id], --PMO_Group_ID created
concat(caseNo,src_id) as [CaseNo],-- Case Number, unique per facility
caseNo as [PMO_number],-- PMO Number
'usei740' as [SrcOrgCode],-- source
'pcc_staging_db58899' as  [STgOrgCode], --staging DB, same for all facilities within one PMO
'usei1104' as [DstOrgCode],--destination
'usei1104' as [DstOrgCodeProd], --org code of current destination
src_id as [SrcFacID],--
src_id as [StgFacID],--
dest_id as [DstFacID],--
...
0 as [PreCaseFac], --0 = No,1 = Yes
0 as [PreCaseBed], --0 = No,1 = Yes
'E14,E15,E1,E2,E2a,E3,E4,E5,E6,E7a,E7b,E7c,E8,E12a,E12b,E11,E9,E10,E13,E16,E20,E22' as [ModList],
0 as [StagingCompleted], -- 0 not completed, 1 = completed
0 as [Completed], -- 0 not completed, 1 = completed
...
select * from LoadEIMaster_Automation where PMO_Number = '58899'

select * from LoadEIMaster_Automation where PMO_Group_Id=1742
/*
