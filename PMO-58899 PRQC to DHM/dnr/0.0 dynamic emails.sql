...
BEGIN
	...
	DECLARE @pmo_number VARCHAR(MAX) = '58899'

	DECLARE @src_org_code VARCHAR(MAX) = 'PRQC'
	DECLARE @dst_org_code VARCHAR(MAX) = 'DHM'

	DECLARE @src_prod VARCHAR(MAX) = '[pccsql-use2-prod-w24-cli0019.ce22455c967a.database.windows.net].[us_prqc_multi]'
	DECLARE @dst_prod VARCHAR(MAX) = '[pccsql-use2-prod-w22-cli0007.4c4638f8e26f.database.windows.net].[us_dhm_multi]'

	DECLARE @src_test_org_code VARCHAR(MAX) = 'USEI1029'
	DECLARE @dst_test_org_code VARCHAR(MAX) = 'USEI41'

	DECLARE @golive_date VARCHAR(MAX) = 'April, 6, 2022'
	DECLARE @golive_day VARCHAR(MAX) = 'Wednesday'
	DECLARE @golive_starttime VARCHAR(MAX) = 'Wednesday, April 6th, 10:00 AM'
	DECLare @golive_endtime varchar(max) = '4:00 AM'
	DECLARE @golive_time VARCHAR(MAX) = '10:00 AM to 4:00 AM'
	...
	DECLARE @all_SRC_facID VARCHAR(MAX) = '1,2,5,'
	DECLARE @all_DST_facID VARCHAR(MAX) = '192,193,194,'
	DECLARE @case_number VARCHAR(MAX) = '1995861' -- netsuite photos & online docs case

	DECLARE @tst VARCHAR(MAX) = '1099'

	DECLARE @srcRegionID varchar(5) = '1'
	DECLARE @dstRegionID varchar(5) = '1'
	...
	DECLARE @deadline varchar(200) = 'Wednesday, April 6th'
	...
	DECLARE @recepients AS NVARCHAR(max) = 'gunald@pointclickcare.com'
