...
BEGIN
	...
	SET @vPMOGroupID = 1763

	/*------Job Fail Email Recipient------*/
	SET @vToEmailAddress =  N'dinesh.gunalapan@pointclickcare.com;TechServicesCustomerMaintenance@pointclickcare.com'

	/*------External Email Notification Recipient, for start of the run, email address only, semicolon separated------*/
	SET @EmailrecipientsExt = 'Ashlee.Moss@pointclickcare.com;Suzette.Edgar@pointclickcare.com;Keith.Tomaszewski@pointclickcare.com;Adrian.Rizos@pointclickcare.com;TSFacAcqConfig@pointclickcare.com;melodee.mercado@pointclickcare.com;Laurie.Watkins@pointclickcare.com;jstroder@daashc.com;lalaran@daashc.com;'
	...
	SET @action_codes_mapping_table			= '[vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO59013_ActionCodes$]'
	SET @status_codes_mapping_table			= '[vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO59013_StatusCodes$]'
	SET @upload_categories_mapping_table	= '[vmuspassvtsjob1.pccprod.local].FacAcqMapping.[dbo].[PMO59013_UploadCategories$]'
	SET @room_rate_type_mapping_table		= '[vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO59013_RoomRateType$]'
	/*------Medicare A id from payer mapping, this is used in room rate type post update------*/
	SET @Medicare_A_in_destination			= '4'

	/*------If 'Y', payer mapping is inculuded and with naming convention, on [vmuspassvtsjob1.pccprod.local].FacAcqMapping: 
			PMO[PMO_NUMBER]_EICase[CASE_NUMBER]_payermapping_fac_[SRC_FAC_ID]_to_[DST_FAC_ID]-----*/
	SET @if_include_payer_mapping			= 'Y'

	SET @prefix_for_UDA						= 'ZION-'
	SET @prefix_for_Care_Plan_Library		= 'ZION-'
	SET @CP_Library_to_exclude_from_prefix	= '8,9'
	SET @suffix_for_order_libraries			= '_'
	SET @suffix_for_sec_user_loginname		= 'ZION'
	...
	BEGIN TRY
		...
		SET @SqlCommand = '
		SELECT DISTINCT RunID,CaseNo,srcfacid,dstfacid
		...
		BEGIN
			...
			BEGIN --Start CurCase
				...
				SET @SqlCommand = @SqlCommand + 'exec  [operational].[sproc_facacq_autopre_RemoveTableFromDataCopy] ''as_ard_adl_keys'''+CHAR(13)+CHAR(10)+CHAR(13)+CHAR(10)
				...
				SET @SqlCommand = @SqlCommand + ',@payer_mapping_bkp = ''[vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.PMO' + @vPMONumber + '_EICase' + @vCaseNo + '_payermapping_fac_' + @vSrcFacId + '_to_' + @vDstFacId + ''''+CHAR(13)+CHAR(10)+CHAR(13)+CHAR(10) END
...
test_usei1058 - DB Version: 4.4.7.3_Z                                         

pcc_staging_db59013 - DB Version: 4.4.7.3_Z                                         

us_srz_multi - DB Version: 4.4.7.3_Z                                         

644	590131	29	29	E1: Resident Identifier, Contact, E2: Staff, E2a: Medical Professional, E3: External Facility, E4: User Defined Data, E5: Room and Bed, E6: Census, E7a: MDS 2, E7b: MDS 3, E7c: Custom UDAs, E8: Diagnosis, E12a: Custom Care Plan, E12b: Care Plan Copy Library, E11: Immunization, E9: Progress Note, E10: Vital, E13: Phys Orders, E16: Alert, E17: Risk Management, E18: Trust, E20: Upload Files, E21: Results Lab, Radiology, E22: Master Insurance, E23: Notes	E14: Security Roles, E15: Security Users, E7d: MMQ, E7e: MMA, E19a: IRM, E19b: IRM
646	590132	32	30	E1: Resident Identifier, Contact, E2: Staff, E2a: Medical Professional, E3: External Facility, E4: User Defined Data, E5: Room and Bed, E6: Census, E7a: MDS 2, E7b: MDS 3, E7c: Custom UDAs, E8: Diagnosis, E12a: Custom Care Plan, E12b: Care Plan Copy Library, E11: Immunization, E9: Progress Note, E10: Vital, E13: Phys Orders, E16: Alert, E17: Risk Management, E18: Trust, E20: Upload Files, E21: Results Lab, Radiology, E22: Master Insurance, E23: Notes	E14: Security Roles, E15: Security Users, E7d: MMQ, E7e: MMA, E19a: IRM, E19b: IRM
...
************************************
****************************************
Prepare Source Backup and Restore Script
****************************************
***************************
Step 2: Setup Source Restore with SP
***************************
*************************************************************
...
***********************************************
***************************************************************
Case No: 590131 : Execute PreMergeScriptSrc on Source DB
***************************************************************
***************************************************************
Case No: 590131 : Execute PreMergeScriptSrc on Source DB
***************************************************************
********************************************************************
Case No: 590131 : Execute PreMergeScriptDst on Destination DB
********************************************************************
************************************************************
Case No: 590131 : Execute PreMergeScript on Staging DB
************************************************************
*******************************************************************
Case No: 590131 : Source to Staging Step3 and Step4 Execution
*******************************************************************
*************************************************************
Case No: 590131 : Execute PostMergeScript on Staging DB
*************************************************************
*************************************************************************************************
Case No: 590131 : Backup Staging DB for TS Team Use Only (In Case):- pcc_staging_db59013
*************************************************************************************************
***************************************************************
Case No: 590132 : Execute PreMergeScriptSrc on Source DB
***************************************************************
***************************************************************
Case No: 590132 : Execute PreMergeScriptSrc on Source DB
***************************************************************
********************************************************************
Case No: 590132 : Execute PreMergeScriptDst on Destination DB
********************************************************************
************************************************************
Case No: 590132 : Execute PreMergeScript on Staging DB
************************************************************
*******************************************************************
Case No: 590132 : Source to Staging Step3 and Step4 Execution
*******************************************************************
*************************************************************
Case No: 590132 : Execute PostMergeScript on Staging DB
*************************************************************
*************************************************************************************************
Case No: 590132 : Backup Staging DB for TS Team Use Only (In Case):- pcc_staging_db59013
*************************************************************************************************
********************************************
Backup Staging DB :- pcc_staging_db59013
********************************************
*******************************************************
Kickoff Staging to Destination Job :- EI_Prepare_Destination__59013
*******************************************************
**************************************************************
...
Completion time: 2022-01-10T10:04:16.8744849-05:00
