-- add this in step 12 of job EI_Prepare_Staging__008001
...
update sec_role 
set description = 'WWHCC-' + description 
--select * 
from sec_role 
where (system_field <> 'Y' or system_field is null)
and description not like 'WWHCC%'


--template_description fix
select * from dbo.pho_std_order where template_description like '%Tuberculin Skin Test (TST) - Results Step 1%'
	
UPDATE dbo.pho_std_order
SET template_description = 'Tuberculin Skin Test (TST) - Results Step 1'
WHERE std_order_id =  101793
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_AdminPicklist$] a
LEFT JOIN [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_AdminPicklist$] b ON a.Map_DstItemId = b.dst_Item_Id
LEFT JOIN test_usei1175.dbo.common_code AS src on src.item_id = a.src_item_id
LEFT JOIN [pccsql-use2-prod-w28-cli0002.874bf44c721a.database.windows.net].us_mhco_multi.dbo.common_code AS dst on dst.item_id = b.dst_item_id
WHERE 
	(
		...
		AND a.Map_DstItemId NOT IN --remove any dupilicate many to one mappings (Any If_merged not As_is or N)		--
			(
				...
				(
					...
					FROM [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_AdminPicklist$]
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_ResidentIdentifier$] a
LEFT JOIN [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_ResidentIdentifier$] b ON a.map_dst_typeid = b.dstIdTypeId
LEFT JOIN [pccsql-use2-prod-ssv-tscon06.b4ea653240a9.database.windows.net].test_usei1175.dbo.id_type AS src on src.id_type_id = a.srcIdTypeId
LEFT JOIN [pccsql-use2-prod-w28-cli0002.874bf44c721a.database.windows.net].us_mhco_multi.dbo.id_type AS dst on dst.id_type_id = b.dstIdTypeId
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_UserDefinedData$] a
LEFT JOIN [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_UserDefinedData$] b ON a.map_dst_typeid = b.dstFieldTypeId
LEFT JOIN [pccsql-use2-prod-ssv-tscon06.b4ea653240a9.database.windows.net].test_usei1175.dbo.user_field_types AS src on src.field_type_id = a.srcFieldTypeId
LEFT JOIN [pccsql-use2-prod-w28-cli0002.874bf44c721a.database.windows.net].us_mhco_multi.dbo.user_field_types AS dst on dst.field_type_id = b.dstFieldTypeId
WHERE  a.srcFieldName IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dst_typeid IS NOT NULL --total count of mapping provided by implementer  
...
----SELECT a.item_code, a.src_Item_Id, a.src_Item_Description, b.dst_Item_Id, b.dst_Item_Description
----SELECT a.pick_list_name,a.src_Item_Description,b.dst_Item_Description
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_Clinical_common_code$] a
LEFT JOIN [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_Clinical_common_code$] b ON a.Map_DstItemId = b.dst_Item_Id
LEFT JOIN test_usei1175.dbo.common_code AS src on src.item_id = a.src_item_id
LEFT JOIN [pccsql-use2-prod-w28-cli0002.874bf44c721a.database.windows.net].us_mhco_multi.dbo.common_code AS dst on dst.item_id = b.dst_item_id
WHERE 
	(
		...
		AND a.Map_DstItemId NOT IN --remove any dupilicate many to one mappings (Any If_merged not As_is or N)		--
			(
				...
				(
					select map_dstitemid 
					FROM [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_Clinical_common_code$]
					WHERE pick_list_name IS NOT NULL -- total mapping provided by analyst to implementer	
						AND map_dstitemid IS NOT NULL --total count of mapping provided by implementer		
						AND src_item_id IS NOT NULL --eliminate any incorrect provided mappings	
						AND ISNUMERIC(map_dstitemid) = 1
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_Clinical_Advanced$] a
inner join [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_Clinical_Advanced$] b 
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_Clinical_Advanced$] a
LEFT JOIN [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_Clinical_Advanced$] b ON a.Map_DstItemId = b.dst_id
LEFT JOIN [pccsql-use2-prod-ssv-tscon06.b4ea653240a9.database.windows.net].test_usei1175.dbo.pho_administration_record AS src on src.administration_record_id = a.src_id
LEFT JOIN [pccsql-use2-prod-w28-cli0002.874bf44c721a.database.windows.net].us_mhco_multi.dbo.pho_administration_record AS dst on dst.administration_record_id = b.dst_id
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_Clinical_Advanced$] a
LEFT JOIN [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_Clinical_Advanced$] b ON a.Map_DstItemId = b.dst_id
LEFT JOIN [pccsql-use2-prod-ssv-tscon06.b4ea653240a9.database.windows.net].test_usei1175.dbo.pho_order_type AS src on src.order_type_id = a.src_id
LEFT JOIN [pccsql-use2-prod-w28-cli0002.874bf44c721a.database.windows.net].us_mhco_multi.dbo.pho_order_type AS dst on dst.order_type_id = b.dst_id
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_Clinical_Advanced$] a
LEFT JOIN [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_Clinical_Advanced$] b ON a.Map_DstItemId = b.dst_id
LEFT JOIN [pccsql-use2-prod-ssv-tscon06.b4ea653240a9.database.windows.net].test_usei1175.dbo.pn_type AS src on src.pn_type_id = a.src_id
LEFT JOIN [pccsql-use2-prod-w28-cli0002.874bf44c721a.database.windows.net].us_mhco_multi.dbo.pn_type AS dst on dst.pn_type_id = b.dst_id
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_Clinical_Advanced$] a
LEFT JOIN [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_Clinical_Advanced$] b ON a.Map_DstItemId = b.dst_id
LEFT JOIN [pccsql-use2-prod-ssv-tscon06.b4ea653240a9.database.windows.net].test_usei1175.dbo.cr_std_immunization AS src on src.std_immunization_id = a.src_id
LEFT JOIN [pccsql-use2-prod-w28-cli0002.874bf44c721a.database.windows.net].us_mhco_multi.dbo.cr_std_immunization AS dst on dst.std_immunization_id = b.dst_id
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_Clinical_Advanced$] a
LEFT JOIN [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_Clinical_Advanced$] b ON a.Map_DstItemId = b.dst_id
LEFT JOIN [pccsql-use2-prod-ssv-tscon06.b4ea653240a9.database.windows.net].test_usei1175.dbo.cp_std_shift AS src on src.std_shift_id = a.src_id
LEFT JOIN [pccsql-use2-prod-w28-cli0002.874bf44c721a.database.windows.net].us_mhco_multi.dbo.cp_std_shift AS dst on dst.std_shift_id = b.dst_id
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
...
from [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_Clinical_Advanced$] a
LEFT JOIN [vmuspassvtsjob1.pccprod.local].FacAcqMapping.dbo.[PMO58784_Clinical_Advanced$] b ON a.Map_DstItemId = b.dst_id
LEFT JOIN [pccsql-use2-prod-ssv-tscon06.b4ea653240a9.database.windows.net].test_usei1175.dbo.inc_std_pick_list_item AS src on src.pick_list_item_id = a.src_id
LEFT JOIN [pccsql-use2-prod-w28-cli0002.874bf44c721a.database.windows.net].us_mhco_multi.dbo.inc_std_pick_list_item AS dst on dst.pick_list_item_id = b.dst_id
WHERE  a.src_desc IS NOT NULL -- total mapping provided by analyst to implementer    
AND a.map_dstitemid IS NOT NULL --total count of mapping provided by implementer  
